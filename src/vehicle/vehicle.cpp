...
auto make_chargeable_battery_buckets(const vehicle& veh) -> battery_charge_buckets {
    ...
    for (const auto part_index : veh.battery_parts) {
        ...
        if (!part.is_available() || part.ammo_capacity() <= part.ammo_remaining()) { continue; }
...
auto make_dischargeable_battery_buckets(const vehicle& veh) -> battery_charge_buckets {
    ...
    for (const auto part_index : veh.battery_parts) {
        const auto& part = veh.cpart(part_index);
        if (!part.is_available() || part.ammo_remaining() <= 0) { continue; }
...
public:
    ...
    virtual detached_ptr<item> add_item_or_charges(
        const tripoint_bub_ms& loc, detached_ptr<item>&& it, bool permit_oob) = 0;
...
public:
    ...
    void unboard(const tripoint_bub_ms& loc) override { g->m.unboard_vehicle(loc); }
    detached_ptr<item> add_item_or_charges(
        const tripoint_bub_ms& loc, detached_ptr<item>&& it, bool /*permit_oob*/) override {
    ...
    void removed(vehicle& veh, const int part) override {
        ...
        if (act.id() == ACT_VEHICLE && act.moves_left > 0 && act.values.size() > 6) {
            if (veh_pointer_or_null(here.veh_at(
                    tripoint_bub_ms(act.values[0], act.values[1], player_character.bub_pos().z())))
                == &veh) {
...
public:
    ...
    detached_ptr<item> add_item_or_charges(
        ...
        if (!m.inbounds(loc)) {
            ...
            if (!permit_oob) { return std::move(it); }
            auto copy = loc;
...
private:
    ...
    auto to_omt(const tripoint_bub_ms& loc, const bool permit_oob) const
        ...
        const bool inbounds = raw.x >= 0 && raw.x <= max_x && raw.y >= 0 && raw.y <= max_y;
        if (!inbounds && !permit_oob) { return std::nullopt; }
...
public:
    ...
    auto add_item_or_charges(const tripoint_bub_ms& loc, detached_ptr<item>&& it, bool permit_oob)
        -> detached_ptr<item> override {
        const auto omt_loc = to_omt(loc, permit_oob);
        if (!omt_loc) { return std::move(it); }
...
vehicle_stack::iterator vehicle_stack::erase(
    vehicle_stack::const_iterator it, detached_ptr<item>* out) {
        ...
        if (proto.color_palette.is_valid()) {
            ...
            for (vehicle_part& part : proto.blueprint->parts) {
                ...
                if (proto.color_match.contains(real_part.id.str())) {
                    ...
                    real_part.set_color(
                        old_bg == RGBColor{} ? c : old_bg, old_fg == RGBColor{} ? c : old_fg);
                }
            }
        } else {
            for (vehicle_part& part : proto.blueprint->parts) { parts.emplace_back(part, this); }
        }
...
bool vehicle::player_in_control(const Character& who) const {
    ...
    if (tags.contains("IN_CONTROL_OVERRIDE")) { return true; }
...
void vehicle::add_missing_frames() {
    ...
    for (auto& i : parts) {
        if (locations_checked.contains(i.mount)) { continue; }
...
void vehicle::add_steerable_wheels() {
    ...
    for (auto& wheel : wheels) { parts[wheel.first].id = wheel.second; }
}

void vehicle::init_state(
    const int init_veh_fuel, const int init_veh_status, const std::optional<bool> locked,
    const std::optional<bool> has_keys) {
    // vehicle parts excluding engines are by default turned off
    for (auto& pt : parts) { pt.enabled = pt.base->is_engine(); }
    ...
    if (init_veh_fuel == -1) { veh_fuel_mult = rng(1, 7); }
    if (init_veh_fuel > 100) { veh_fuel_mult = 100; }
    ...
    if (get_avail_parts(VPFLAG_CONTROLS).part_count() > 0) {
        ...
        if (has_keys.has_value()) {
            ...
            for (const auto& vp : get_all_parts()) {
                if (std::ranges::any_of(vs_NO_HOTWIRING, [&](const std::string& flag) {
                        return vp.has_feature(flag);
                    })) {
                    needsHotwire = false;
                    break;
                }
    ...
    if (veh_status != 0) {
        ...
        if (veh_fuel_mult > 0 && !get_avail_parts("ENGINE").empty() && one_in(current_day + 4)
            && !destroyEngine && !needsHotwire && has_engine_type_not(fuel_type_muscle, true)) {
            engine_on = true;
        }
        ...
        if (one_in(10)) { blood_covered = true; }

        if (one_in(8)) { blood_inside = true; }

        for (const vpart_reference& vp : get_parts_including_carried("FRIDGE")) {
            if (one_in(2)) { vp.part().enabled = true; }
        }
    ...
    for (const vpart_reference& vp : get_all_parts()) {
        ...
        if (pt.is_reactor()) {
            if (veh_fuel_mult == 100) { // Mint condition vehicle
                ...
                pt.ammo_set(itype_plut_cell,
                            pt.ammo_capacity() * (veh_fuel_mult + rng(0, 10)) / 100);
        ...
        if (pt.is_tank() && !type->parts[p].fuel.is_null()) {
            ...
            if (combined_rate < 1.0f) {
                if (rng_float(0, 1) < combined_rate) { pt.ammo_set(type->parts[p].fuel, qty); }
            } else {
                auto scaled_qty =
                    std::min(static_cast<int>(qty * combined_rate), pt.ammo_capacity());
        ...
        if (vp.has_feature("OPENABLE")) { // doors are closed
            if (!pt.open && one_in(4)) {
                ...
                if (lock) { lock->part().enabled = false; }
            }
        }
        ...
        if (veh_status == 0) {
            ...
            if (roll < unhurt) {
                if (roll <= broken) {
                    ...
                    set_hp(pt, (roll - broken) / static_cast<double>(unhurt - broken)
                                   * vp.info().durability);
                }
            } else {
                set_hp(pt, vp.info().durability);
            ...
            if (vp.has_feature(VPFLAG_ENGINE)) {
                ...
                if (destroyEngine && pt.faults_potential().empty()) {
                    ...
                    do { pt.fault_set(random_entry(pt.faults_potential())); } while (one_in(3));
                }

            } else if (
                (destroySeats && (vp.has_feature("SEAT") || vp.has_feature("SEATBELT")))
            ...
            if (vp.has_feature("SOLAR_PANEL") && one_in(4)) { set_hp(pt, 0); }

            // An added 5% chance to bust each windshield
            if (vp.has_feature("WINDSHIELD") && one_in(20)) { set_hp(pt, 0); }
            ...
            if (blood_inside) {
                ...
                if (blood_inside_pos) {
                    const int distSq =
                        std::pow(blood_inside_pos->x() - vp.mount().x(), 2)
                        + std::pow(blood_inside_pos->y() - vp.mount().y(), 2);
                    if (distSq <= 1) { pt.blood = rng(200, 400) - distSq * 100; }
...
void vehicle::autopilot_patrol() {
    ...
    const auto& zone_src_set =
        mgr.get_near(zone_type_id("VEHICLE_PATROL"), abs_ms_location(), g_max_view_distance);
    ...
    const tripoint_abs_ms max_tri =
        x_side ? tripoint_abs_ms(point_along, max.y(), min.z())
               : tripoint_abs_ms(max.x(), point_along, min.z());
    const tripoint_abs_ms min_tri =
        x_side ? tripoint_abs_ms(point_along, min.y(), min.z())
...
std::set<point_abs_ms> vehicle::immediate_path(units::angle rotate) {
    ...
    std::vector<point_abs_ms> front_row = line_to(bub_to_abs(fl_bub).xy(), bub_to_abs(fr_bub).xy());
    for (point_abs_ms elem : front_row) {
        for (int i = 0; i < distance_to_check; ++i) {
            collision_vector.advance(i);
            point_abs_ms point_to_add =
                elem + point_rel_ms(collision_vector.dx(), collision_vector.dy());
...
static int get_turn_from_angle(
    const units::angle angle, const tripoint_abs_ms& vehpos, const tripoint_abs_ms& target,
    bool reverse = false) {
...
void vehicle::drive_to_local_target(const tripoint_abs_ms& target, bool follow_protocol) {
    ...
    for (auto pt_elem : points_to_check) {
        auto elem = abs_to_bub(pt_elem);
        if (stop) { break; }
        const optional_vpart_position ovp = g->m.veh_at(tripoint_bub_ms(elem, abs_sm_pos.z()));
        if (g->m.impassable_ter_furn(tripoint_bub_ms(elem, abs_sm_pos.z()))
            || (ovp && &ovp->vehicle() != this)) {
            stop = true;
            break;
        }
    ...
    if (stop) {
        if (autopilot_on) {
            ...
            se.description =
                string_format(_("the %s emitting a beep and saying \"Obstacle detected!\""), name);
            sounds::sound(se);
        }
        stop_autodriving();
        return;
    ...
    if (follow_protocol) {
        ...
        if ((turn_x > 0 || turn_x < 0) && velocity > 447) { accel_y = 1; }
        if ((velocity < std::min(safe_velocity(), is_aircraft() && is_flying_in_air() ? 5364 : 1431)
             && turn_x == 0)
            || velocity < 224) {
            accel_y = -1;
        }
        if (is_patrolling && velocity > 179) { accel_y = 1; }
    }
    selfdrive(point(turn_x, accel_y));
}

tripoint_abs_ms vehicle::get_autodrive_target() { return autodrive_local_target; }
 ...
 */
void vehicle::smash(
    map& m, float hp_percent_loss_min, float hp_percent_loss_max, float percent_of_parts_to_affect,
    tripoint_rel_ms damage_origin, float damage_size) {
    for (auto& part : parts) {
        // Skip any parts already mashed up or removed.
        if (part.is_broken() || part.removed || part.info().has_flag(VPFLAG_NOSMASH)) { continue; }
        ...
        if (roll < pct_af) {
            double dist =
                damage_size == 0.0f
                    ? 1.0f
                    : clamp(1.0f - trig_dist(damage_origin.xy(), part.precalc[0]) / damage_size,
                            0.0f, 1.0f);
            // Everywhere else, drop by 10-120% of max HP (anything over 100 = broken)
            if (mod_hp(part,
                       0
                           - (rng_float(hp_percent_loss_min * dist, hp_percent_loss_max * dist)
                              * part.info().durability),
                       DT_BASH)) {
                part.ammo_unset();
            }
        }
    ...
    for (int p = static_cast<int>(parts.size()) - 1; p >= 0; p--) {
        ...
        for (int other_i = static_cast<int>(parts_here.size()) - 1; other_i >= 0; other_i--) {
            ...
            if (p_info.get_id() == other_p_info.get_id()
                ...
                // Deferred creation of the handler to here so it is only created when actually
                // needed.
                if (!handler_ptr) {
                    // This is a heuristic: we just assume the default handler is good enough when
                    // called on the main game map. And assume that we run from some mapgen code if
...
auto vehicle::smash(
    mapgen_constructor& m, float hp_percent_loss_min, float hp_percent_loss_max,
    float percent_of_parts_to_affect, tripoint_rel_ms damage_origin, float damage_size) -> void {
    for (auto& part : parts) {
        if (part.is_broken() || part.removed || part.info().has_flag(VPFLAG_NOSMASH)) { continue; }
...
void vehicle::toggle_specific_part(int p, bool on) { parts[p].enabled = on; }
...
bool vehicle::can_enable_muscle_engine(int e, std::string& failure_reason) const {
    ...
    if (passenger != nullptr) {
        if (engine_info.has_flag("MUSCLE_LEGS") && passenger->get_working_leg_count() < 2) {
            failure_reason = string_format(
                _("%s cannot operate the %s due to injured legs."), passenger->name,
                engine_info.name());
            return false;
        }
        if (engine_info.has_flag("MUSCLE_ARMS") && passenger->get_working_arm_count() < 2) {
            failure_reason = string_format(
                _("%s cannot operate the %s due to injured arms."), passenger->name,
                engine_info.name());
            return false;
        }
        return true;
    ...
    failure_reason =
        string_format(_("The %s cannot operate without an occupant."), engine_info.name());
    return false;
}
...
bool vehicle::has_engine_type(const itype_id& ft, const bool enabled) const {
    for (size_t e = 0; e < engines.size(); ++e) {
        if (is_engine_type(e, ft) && (!enabled || is_engine_on(e))) { return true; }
    }
    return false;
}
...
bool vehicle::has_engine_conflict(
    const vpart_info* possible_conflict, std::string& conflict_type) const {
    ...
    if (new_excludes.empty()) { return false; }
    ...
    for (int engine : engines) {
        ...
        std::set_intersection(
            new_excludes.begin(), new_excludes.end(), install_excludes.begin(),
...
bool vehicle::is_engine_type(const int e, const itype_id& ft) const {
    ...
    bool engine_has_fuel =
        std::find(engine_fuel.begin(), engine_fuel.end(), ft) != engine_fuel.end();
    return parts[engines[e]].ammo_current().is_null()
             ? engine_has_fuel
...
bool vehicle::is_alternator_on(const int a) const {
    ...
    if (alt.is_unavailable()) { return false; }
...
void vehicle::backfire(const int e) const {
    ...
    se.description = string_format(
        _("a loud BANG! from the %s"), // NOLINT(cata-text-style)
...
const vpart_info& vehicle::part_info(int index, bool include_removed) const {
    if (index < static_cast<int>(parts.size())) {
        if (!parts[index].removed || include_removed) { return parts[index].info(); }
    }
...
int vehicle::part_vpower_w(const int index, const bool at_full_hp) const {
    ...
    if (part_flag(index, VPFLAG_ENGINE)) {
        if (pwr == 0) { pwr = vhp_to_watts(vp.base->engine_displacement()); }
        if (vp.info().fuel_type == fuel_type_animal) {
        ...
        if (vp.info().fuel_type == fuel_type_wind) {
            ...
            if (windpower < 1) {
                ...
                // For gameplay purposes, permit adjusting sails enough to sail upwind so long as
                // it's blowing at all.
                rl_vec2d windvec;
...
bool vehicle::is_structural_part_removed() const {
    for (const vpart_reference& vp : get_all_parts()) {
        if (vp.part().removed && vp.info().location == part_location_structure) { return true; }
    }
    return false;
}
...
bool vehicle::can_mount(const tripoint_mnt_veh& dp, const vpart_id& id) const {
    ...
    if (part.has_flag("NOINSTALL")) { return false; }
    ...
    if (parts_in_square.empty() && part.location != part_location_structure) { return false; }
    ...
    for (const auto& elem : parts_in_square) {
        ...
        if (part.has_flag("CARGO") && other_part.has_flag("CARGO")) { return false; }
    }
    ...
    if (!parts.empty()) {
        if (!is_structural_part_removed() && !has_structural_part(dp)
            && !has_structural_part(dp + point_east) && !has_structural_part(dp + point_south)
            && !has_structural_part(dp + point_west) && !has_structural_part(dp + point_north)) {
            return false;
        }
    }
    ...
    std::string empty;
    if (has_engine_conflict(&part, empty)) { return false; }
    ...
    if (part.has_flag("VISION") && !part.has_flag("CAMERA")) {
        for (const auto& elem : parts_in_square) {
            if (part_info(elem).has_flag("OPAQUE")) { return false; }
        }
    }
    // Opaque parts cannot be mounted on mirrors parts
    if (part.has_flag("OPAQUE")) {
        for (const auto& elem : parts_in_square) {
            if (part_info(elem).has_flag("VISION") && !part_info(elem).has_flag("CAMERA")) {
                return false;
            }
        }
    }
    ...
    if (part.has_flag("TURRET_MOUNT")) {
        for (const auto& elem : parts_in_square) {
            if (part_info(elem).has_flag("TURRET_MOUNT")) { return false; }
        }
    }
    ...
    if (part.has_flag("TURRET_CONTROLS") || part.has_flag("LASER_DESIGNATOR")) {
        for (const auto& elem : parts_in_square) {
            if (part_info(elem).has_flag("MANUAL")) { return false; }
        }
    }
...
bool vehicle::can_unmount(const int p, std::string& reason) const {
    if (p < 0 || p > static_cast<int>(parts.size())) { return false; }
    ...
    if (part_info(p).location == part_location_structure
        ...
        if (parts_in_square.size() == 1) {
            ...
            for (int i = 0; i < 4; i++) {
                const auto next =
                    parts[p].mount
                    + point_rel_veh(i < 2 ? (i == 0 ? -1 : 1) : 0, i < 2 ? 0 : (i == 2 ? -1 : 1));
 ...
 */
bool vehicle::is_connected(
    const vehicle_part& to, const vehicle_part& from, const vehicle_part& excluded_part) const {
...
int vehicle::install_part(const tripoint_mnt_veh& dp, const vpart_id& id, bool force) {
    if (!(force || can_mount(dp, id))) { return -1; }
    detached_ptr<item> obj = item::spawn(id.obj().item);
    int ret = install_part(dp, vehicle_part(id, dp, std::move(obj), this));
    return ret;
}

int vehicle::install_part(
    const tripoint_mnt_veh& dp, const vpart_id& id, detached_ptr<item>&& obj, bool force) {
    if (!(force || can_mount(dp, id))) { return -1; }
...
int vehicle::install_part(const tripoint_mnt_veh& dp, vehicle_part&& new_part) {
    ...
    if (new_part.is_engine()) {
        ...
        static const std::vector<std::string> enable_like = {
            {"CONE_LIGHT",   "CIRCLE_LIGHT", "AISLE_LIGHT",    "AUTOPILOT", "DOME_LIGHT",
             "ATOMIC_LIGHT", "STEREO",       "CHIMES",         "FRIDGE",    "FREEZER",
             "RECHARGE",     "PLOW",         "REAPER",         "PLANTER",   "SCOOP",
             "SPACE_HEATER", "COOLER",       "WATER_PURIFIER", "ROCKWHEEL", "ROADHEAD",
             "CONVERTER"}};
...
bool vehicle::merge_rackable_vehicle(vehicle* carry_veh, const std::vector<int>& rack_parts) {
    ...
    for (auto carry_part : carry_veh_structs) {
        ...
        for (int rack_part : rack_parts) {
            size_t j = 0;
            // There's no mathematical transform from global pos3 to vehicle mount, so search for
            // the carry part in global pos3 after translating
            // TODO: Check if still true. bubble_to_mount exists, and global pos3 used to refer to
            // bubble space position bubble_to_mount( carry_pos );
            tripoint_mnt_veh carry_mount;
            for (point offset : four_cardinal_directions) {
                carry_mount = parts[rack_part].mount + offset;
                auto possible_pos = mount_to_bubble(carry_mount);
                if (possible_pos.raw() == carry_pos.raw()) { break; }
                ++j;
            }
    ...
    if (found_all_parts) {
        ...
        for (const auto& carry_map : carry_data) {
            std::string offset = string_format(
                "%s%3d", carry_map.old_mount == tripoint_mnt_veh::zero() ? axis : " ",
                axis == "X" ? carry_map.old_mount.x() : carry_map.old_mount.y());
            std::string unique_id = string_format(
                "%s%3d%s", offset, static_cast<int>(to_degrees(relative_dir)), carry_veh->name);
            for (int carry_part : carry_map.carry_parts_here) {
                ...
                // give each carried part a tracked_flag so that we can re-enable overmap tracking
                // on unloading if necessary
                if (carry_veh->tracking_on) { carried_part.set_flag(vehicle_part::tracked_flag); }
            ...
            const std::pair<
                std::unordered_multimap<tripoint_mnt_veh, zone_data>::iterator,
                std::unordered_multimap<tripoint_mnt_veh, zone_data>::iterator>
                zones_on_point = carry_veh->loot_zones.equal_range(carry_map.old_mount);
            for (std::unordered_multimap<tripoint_mnt_veh, zone_data>::const_iterator it =
        ...
        for (auto& new_zone : new_zones) {
            zone_manager::get_manager()
                .create_vehicle_loot_zone(*this, new_zone.first, new_zone.second);
        }
...
bool vehicle::remove_part(const int p, RemovePartHandler& handler) {
    ...
    if (part_flag(p, "ENGINE") && engines.size() > 1) {
        ...
        if (!any_engine_on) {
            engine_on = false;
            for (auto& e : engines) { toggle_specific_part(e, true); }
        }
    }
    ...
    if (iter != labels.end() && parts_at_relative(vp_mount, false).empty()) { labels.erase(iter); }
...
void vehicle::part_removal_cleanup() {
    ...
    for (std::vector<vehicle_part>::iterator it = parts.begin(); it != parts.end(); /* noop */) {
        if (it->removed) {
            ...
            while (!items.empty()) { items.erase(items.begin()); }
...
void vehicle::remove_carried_flag() {
    for (vehicle_part& part : parts) {
        if (part.carry_names.empty()) {
            // note: we get here if the part was added while the vehicle was carried / mounted. This
            // is not expected. still try to remove the carried flag, if any.
            part.remove_flag(vehicle_part::carried_flag);
        } else {
            part.carry_names.pop();
            if (part.carry_names.empty()) { part.remove_flag(vehicle_part::carried_flag); }
        }
    }
}
...
void vehicle::remove_tracked_flag() {
    for (vehicle_part& part : parts) {
        if (part.carry_names.empty()) {
            ...
            part.carry_names.pop();
            if (part.carry_names.empty()) { part.remove_flag(vehicle_part::tracked_flag); }
        }
    }
}

bool vehicle::remove_carried_vehicle(const std::vector<int>& carried_parts) {
    if (carried_parts.empty()) { return false; }
    std::string veh_record;
    tripoint_bub_ms nbp;
    bool x_aligned = false;
    bool tracked_parts = false; // we will set this to true if any of the vehicle parts carry a
                                // tracked_flag
    for (int carried_part : carried_parts) {
        // check if selected part carries tracking flag
        if (parts[carried_part].has_flag(vehicle_part::tracked_flag)) { // this should only need to
                                                                        // run once
            tracked_parts = true;
        }
    ...
    if (veh_record.empty()) { return false; }
    units::angle new_dir = normalize(
        units::from_degrees(std::stoi(veh_record.substr(4, 3))) + face.dir());
    ...
    for (auto carried_part : carried_parts) {
        ...
        if (!parts[carried_part].carry_names.empty()) {
            // the mount string should be something like "X 0 0" for ex. We get the first number out
            // of it.
            mount_str = parts[carried_part].carry_names.top().substr(1, 3);
        } else {
            // FIX #28712; if we get here it means that a part was added to the bike while the
            // latter was a carried vehicle. This part didn't get a carry_names because those are
            // assigned when the carried vehicle is loaded. We can't be sure to which vehicle it
            // really belongs to, so it will be detached from the vehicle. We can at least inform
            // the player that there's something wrong.
            add_msg(
                m_warning,
            ...
            // check if any other parts at the same location have a valid carry name so we can still
            // have a valid mount location.
    ...
    if (success) {
        ...
        if (tracked_parts) {
            ...
            new_vehicle->remove_tracked_flag(); // remove our tracking flags now that the vehicle
                                                // isn't carried
        }
...
bool vehicle::find_and_split_vehicles(int exclude) {
    ...
    for (size_t cnt = 0; cnt < 4; cnt++) {
        ...
        for (auto p : valid_parts) {
            if (parts[p].removed) { continue; }
            if (!checked_parts.contains(p)) {
                test_part = p;
                break;
            }
        }
        if (test_part == -1 || static_cast<size_t>(test_part) > parts.size()) { break; }
        ...
        while (!search_queue.empty()) {
            ...
            if (checked_parts.contains(test_part)) { continue; }
            for (auto p : test_set.second) { veh_parts.push_back(p); }
        ...
        if (cnt > 0) { all_vehicles.push_back(veh_parts); }
    }
...
void vehicle::relocate_passengers(const std::vector<Character*>& passengers) {
    ...
    for (auto* passenger : passengers) {
        for (const vpart_reference& vp : boardables) {
            if (vp.part().passenger_id == passenger->getID()) { passenger->setpos(vp.pos()); }
        }
    }
}
    ...
    for (i = 0; i < new_vehs.size(); i++) {
        ...
        for (size_t new_part = 0; new_part < split_parts.size(); new_part++) {
            ...
            if (!split_mounts.empty()) {
                ...
                new_mount = (cur_mount - mnt_offset).reinterpret_as<tripoint_mnt_veh>();
            }
            ...
            if (part_flag(mov_part, "BOARDABLE")) {
                ...
                if (passenger) { passengers.push_back(passenger); }
            }
            // if this part is a towing part, transfer the tow_data to the new vehicle.
            if (part_flag(mov_part, "TOW_CABLE")) {
                if (is_towed()) {
                    tow_data.get_towed_by()
                        ->tow_data.set_towing(tow_data.get_towed_by(), new_vehicle);
            ...
            // The zone manager will be updated when we next interact with it through
            // get_vehicle_zones
            zones_dirty = true;
        ...
        for (std::pair<tripoint_mnt_veh, zone_data> zone : new_zones) {
            zone_manager::get_manager()
                .create_vehicle_loot_zone(*new_vehicle, zone.first, zone.second);
        }
        ...
        here.set_seen_cache_dirty(tripoint_bub_ms::zero());
        here.set_vehicle_cache_dirty(abs_sm_pos.z());
        if (!new_labels.empty()) { new_vehicle->labels = new_labels; }
        ...
        new_vehicle->precalc_mounts(
            1, new_vehicle->skidding ? new_vehicle->turn_dir : new_vehicle->face.dir(),
            new_vehicle->pivot_point());
        new_vehicle->adjust_zlevel(1, tripoint_rel_ms::zero());
        new_vehicle->shift_zlevel();
        if (!passengers.empty()) { new_vehicle->relocate_passengers(passengers); }
    }
    if (did_split) { part_removal_cleanup(); }
    return did_split;
}
...
item& vehicle::part_base(int p) { return *parts[p].base; }

int vehicle::find_part(const item& it) const {
    auto idx = std::ranges::find_if(parts, [&it](const vehicle_part& e) { return e.base == &it; });
...
std::vector<int> vehicle::parts_at_relative(
    const tripoint_mnt_veh& dp, const bool use_cache) const {
    if (!use_cache) {
        std::vector<int> res;
        for (const vpart_reference& vp : get_all_parts()) {
...
std::optional<vpart_reference> vpart_position::part_displayed() const {
    ...
    if (part_id == -1) { return std::nullopt; }
    return vpart_reference(vehicle(), part_id);
}

std::optional<vpart_reference> vpart_position::part_with_feature(
    const std::string& f, const bool unbroken) const {
...
std::optional<vpart_reference> vpart_position::part_with_feature(
    const vpart_bitflags f, const bool unbroken) const {
    const int i = vehicle().part_with_feature(part_index(), f, unbroken);
    if (i < 0) { return std::nullopt; }
    return vpart_reference(vehicle(), i);
}

std::optional<vpart_reference> optional_vpart_position::part_with_feature(
    const std::string& f, const bool unbroken) const {
...
int vehicle::part_with_feature(int part, vpart_bitflags const flag, bool unbroken) const {
    if (part_flag(part, flag) && (!unbroken || !parts[part].is_broken())) { return part; }
    ...
    if (it != relative_parts.end()) {
        ...
        for (auto& i : parts_here) {
            if (part_flag(i, flag) && (!unbroken || !parts[i].is_broken())) { return i; }
        }
    }
    return -1;
}
...
int vehicle::part_with_feature(
    const tripoint_mnt_veh& pt, const std::string& flag, bool unbroken) const {
    std::vector<int> parts_here = parts_at_relative(pt, true);
    for (auto& elem : parts_here) {
        if (part_flag(elem, flag) && (!unbroken || !parts[elem].is_broken())) { return elem; }
    }
    return -1;
}
...
int vehicle::avail_part_with_feature(int part, vpart_bitflags const flag, bool unbroken) const {
    ...
    if ((part_a >= 0) && parts[part_a].is_available()) { return part_a; }
    return -1;
}
...
int vehicle::avail_part_with_feature(
    const tripoint_mnt_veh& pt, const std::string& flag, bool unbroken) const {
...
int vehicle::obstacle_at_position(const tripoint_mnt_veh& pos) const {
    ...
    if (ref.info().has_flag(VPFLAG_OPENABLE) && ref.open) { return -1; }
...
std::vector<vehicle_part*> vehicle::get_parts_at(
    const tripoint_bub_ms& pos, const std::string& flag, const part_status_flag condition) {
    const auto relative_pos = pos - bub_ms_location();
    ...
    for (auto& e : parts) {
        ...
        if (!e.removed && (flag.empty() || e.info().has_flag(flag))
...
std::vector<const vehicle_part*> vehicle::get_parts_at(
    const tripoint_bub_ms& pos, const std::string& flag, const part_status_flag condition) const {
    const auto relative_pos = pos - bub_ms_location();
...
std::optional<std::string> vpart_position::get_label() const {
    ...
    if (it == vehicle().labels.end()) { return std::nullopt; }
    if (it->text.empty()) {
...
void vpart_position::set_label(const std::string& text) const {
    ...
    // TODO: empty text should remove the label instead of just storing an empty string, see
    // get_label
    if (it == labels.end()) {
...
int vehicle::next_part_to_close(int p, bool outside) const {
    ...
    // We want reverse, since we close the outermost thing first (curtains), and then the innermost
    // thing (door)
    for (std::vector<int>::reverse_iterator part_it = parts_here.rbegin();
         part_it != parts_here.rend(); ++part_it) {

        if (part_flag(*part_it, VPFLAG_OPENABLE) && parts[*part_it].is_available()
...
int vehicle::next_part_to_open(int p, bool outside) const {
    ...
    // We want forwards, since we open the innermost thing first (curtains), and then the innermost
    // thing (door)
    for (auto& elem : parts_here) {
...
vehicle_part_with_feature_range<std::string> vehicle::get_avail_parts(std::string feature) const {
    return vehicle_part_with_feature_range<std::string>(
        const_cast<vehicle&>(*this), std::move(feature),
        (part_status_flag::working | part_status_flag::available));
}

vehicle_part_with_feature_range<vpart_bitflags> vehicle::get_avail_parts(
    const vpart_bitflags feature) const {
    return vehicle_part_with_feature_range<vpart_bitflags>(
        const_cast<vehicle&>(*this), feature,
        (part_status_flag::working | part_status_flag::available));
}

vehicle_part_with_feature_range<std::string> vehicle::get_parts_including_carried(
    std::string feature) const {
    return vehicle_part_with_feature_range<
        std::string>(const_cast<vehicle&>(*this), std::move(feature), part_status_flag::working);
}

vehicle_part_with_feature_range<vpart_bitflags> vehicle::get_parts_including_carried(
    const vpart_bitflags feature) const {
    return vehicle_part_with_feature_range<
        vpart_bitflags>(const_cast<vehicle&>(*this), feature, part_status_flag::working);
}

vehicle_part_with_feature_range<std::string> vehicle::get_any_parts(std::string feature) const {
    return vehicle_part_with_feature_range<
        std::string>(const_cast<vehicle&>(*this), std::move(feature), part_status_flag::any);
}

vehicle_part_with_feature_range<vpart_bitflags> vehicle::get_any_parts(
    const vpart_bitflags feature) const {
    return vehicle_part_with_feature_range<
        vpart_bitflags>(const_cast<vehicle&>(*this), feature, part_status_flag::any);
}

vehicle_part_with_feature_range<std::string> vehicle::get_enabled_parts(std::string feature) const {
    return vehicle_part_with_feature_range<std::string>(
        const_cast<vehicle&>(*this), std::move(feature),
        (part_status_flag::enabled | part_status_flag::working | part_status_flag::available));
}
...
// therefore the part index is now invalid shifted by one or two ( depending on how many other NPCs
// working on this vehicle ) so loop over the part indexes in reverse order to get the next one down
// that matches the part type we wanted to remove
int vehicle::get_next_shifted_index(int original_index, Character& who) {
    ...
    if (!found_shifted_index) {
        // we are probably down to a few parts left, and things get messy here, so an alternative
        // index maybe can't be found if loads of npcs are all removing parts at the same time. if
        // that's the case, just bail out and give up, somebody else is probably doing the job right
        // now anyway.
        return -1;
    } else {
        return ret_index;
...
std::vector<std::vector<int>> vehicle::find_lines_of_parts(int part, const std::string& flag) {
    ...
    if (possible_parts.empty()) { return ret_parts; }
    ...
    for (const vpart_reference& vp : possible_parts) {
        if (vp.part().is_unavailable() || !vp.has_feature("MULTISQUARE")
            || vp.info().get_id() != part_id) {
            continue;
        }
        if (vp.mount().x() == target.x()) { x_parts.push_back(vp.part_index()); }
        if (vp.mount().y() == target.y()) { y_parts.push_back(vp.part_index()); }
    }
    ...
    if (y_parts.size() == 1 && x_parts.size() == 1) { ret_parts.push_back(x_parts); }
    return ret_parts;
}
...
int vehicle::part_at(const tripoint_rel_ms& dp) const {
    for (const vpart_reference& vp : get_all_parts()) {
        ...
        if (!p.removed && p.precalc[0] == dp.xy() && p.mount.z() + p.z_terrain[0] == dp.z()) {
...
int vehicle::index_of_part(const vehicle_part* const part, const bool check_removed) const {
    if (part != nullptr) {
        for (const vpart_reference& vp : get_all_parts()) {
            ...
            if (!check_removed && next_part.removed) { continue; }
            if (part->id == next_part.id && part->mount == vp.mount()) { return vp.part_index(); }
        }
    }
    return -1;
}
...
int vehicle::part_displayed_at(const tripoint_mnt_veh& dp) const {
    ...
    if (parts_in_square.empty()) { return -1; }
    ...
    for (size_t index = 1; index < parts_in_square.size(); index++) {
        if ((part_info(parts_in_square[top_part]).z_order
            ...
            && (part_info(parts_in_square[index]).z_order < hide_z_at_or_above)) {
            top_part = index;
        }
    }
...
int vehicle::roof_at_part(const int part) const {
    ...
    for (const int p : parts_in_square) {
        if (part_info(p).location == "on_roof" || part_flag(p, "ROOF")) { return p; }
    }
...
void vehicle::coord_translate_reverse(
...
tripoint_bub_ms vehicle::mount_to_bubble(
    const tripoint_mnt_veh& mount, const tripoint_rel_veh& offset) const {
...
tripoint_rel_ms vehicle::rotate_to_world(
    units::angle dir, const tripoint_mnt_veh& pivot, const tripoint_mnt_veh& p) const {
    ...
    if (rotation_info[increment].swapXY) { std::swap(result.x, result.y); }
    if (rotation_info[increment].flipH) { result.x = -result.x; }
    if (rotation_info[increment].flipV) { result.y = -result.y; }
    return tripoint_rel_ms(result);
}

tripoint_mnt_veh vehicle::rotate_to_local(
    units::angle dir, const tripoint_mnt_veh& pivot, const tripoint_rel_ms& p) const {
    int increment = angle_to_increment(dir);
...
int vehicle::angle_to_increment(units::angle dir) {
    ...
    if (increment < 0) { increment += 360 / 15; }
    return increment;
}
...
bool vehicle::check_rotated_intervening(
    ...
    if (!((abs(delta.x()) == 2 && abs(delta.y()) == 1)
          || (abs(delta.x()) == 1 && abs(delta.y()) == 2))) { // Check that we're moving like a
                                                              // knight
...
player* vehicle::get_passenger(int p) const {
    ...
    for (auto& part : parts) {
        if (part.removed || part.mount != target_mount) { continue; }
...
monster* vehicle::get_pet(int p) const {
    ...
    if (p >= 0) { return g->critter_at<monster>(bub_part_location(p), true); }
    return nullptr;
}

tripoint_abs_ms vehicle::abs_ms_location() const { return project_combine(abs_sm_pos, sm_ms_pos); }
...
tripoint_bub_ms vehicle::bub_part_location(const vehicle_part& pt) const {
    return bub_ms_location() + tripoint_rel_ms(pt.precalc[0], pt.mount.z() + pt.z_terrain[0]);
}
...
tripoint_abs_ms vehicle::abs_part_location(const vehicle_part& pt) const {
    return abs_ms_location() + tripoint_rel_ms(pt.precalc[0], pt.mount.z() + pt.z_terrain[0]);
}
...
tripoint_mnt_veh vehicle::local_center_of_mass() const {
    if (mass_center_no_precalc_dirty) { calc_mass_center(false); }
...
int vehicle::fuel_left(const itype_id& ftype, bool recurse) const {
    ...
    if (ftype == fuel_type_battery) {
        fl = std::accumulate(
            battery_parts.begin(), battery_parts.end(), 0,
    ...
    if (ftype == fuel_type_muscle) {
        ...
        for (int ep = 0; ep < static_cast<int>(engines.size()); ep++) {
            if (is_engine_type(ep, fuel_type_muscle) && is_engine_on(ep)) {
                if (has_muscle_engine_operator(ep)) { active_operators++; }
            }
        }
        if (active_operators > 0) { fl += 10 * active_operators; }
...
int vehicle::drain(const int index, int amount) {
    ...
    if (pt.ammo_current() == fuel_type_battery) { return drain(fuel_type_battery, amount); }
    if (!pt.is_tank() || !pt.ammo_remaining()) {
        debugmsg("Tried to drain something without any liquid: %s amount: %d ammo: %d", pt.name(),
                 amount, pt.ammo_remaining());
        return 0;
    }
    ...
    for (size_t e = 0; e < engines.size(); ++e) {
        if (is_engine_type_on(e, ftype)) {
            if (parts[engines[e]].ammo_current() == fuel_type_battery
                ...
                if (parts[e].faults().contains(fault_filter_air)) { fcon *= 2; }
            }
        }
    }
    return fcon;
...
int vehicle::consumption_per_hour(const itype_id& ftype, int fuel_rate_w) const {
    ...
    if (fuel_rate_w == 0 || fuel.has_flag(flag_PERPETUAL) || !engine_on) { return 0; }
...
int vehicle::ideal_engine_power(bool safe) const {
    ...
    for (size_t e = 0; e < engines.size(); e++) {
        if (is_engine_on(e)) {
            ...
            if (parts[engines[e]].faults().contains(fault_filter_fuel)) { m2c *= 0.6; }
...
bool vehicle::is_moving() const { return velocity != 0; }
...
int vehicle::ground_acceleration(const bool fueled, int at_vel_in_vmi, const bool ideal) const {
    if (!(engine_on || skidding)) { return 0; }
    int target_cmps = std::max(at_vel_in_vmi, std::max(447, max_velocity(fueled) / 4));
    double weight = to_kilogram(total_mass());
    if (is_towing()) {
        vehicle* other_veh = tow_data.get_towed();
        if (other_veh) { weight = weight + to_kilogram(other_veh->total_mass()); }
    }
    ...
    add_msg(m_debug, "%s: accel at %d cm/s is %d cm/s", name, target_cmps, accel_at_vel);
    return accel_at_vel;
}

int vehicle::aircraft_acceleration(const bool fueled, int at_vel_in_vmi, const bool ideal) const {
    (void)at_vel_in_vmi;
    if (!(engine_on || is_flying)) { return 0; }
...
int vehicle::water_acceleration(const bool fueled, int at_vel_in_vmi, const bool ideal) const {
    if (!(engine_on || skidding)) { return 0; }
    int target_cmps = std::max(at_vel_in_vmi, std::max(447, max_water_velocity(fueled) / 4));
    double weight = to_kilogram(total_mass());
    if (is_towing()) {
        vehicle* other_veh = tow_data.get_towed();
        if (other_veh) { weight = weight + to_kilogram(other_veh->total_mass()); }
    ...
    int engine_power_ratio = (ideal ? ideal_engine_power() : total_power_w(fueled)) / weight;
    int accel_at_vel = 100 * 100 * engine_power_ratio / target_cmps;
    add_msg(m_debug, "%s: water accel at %d cm/s is %d cm/s", name, target_cmps, accel_at_vel);
    return accel_at_vel;
}
...
int vehicle::max_ground_velocity(const bool fueled, const bool ideal) const {
    ...
    double max_in_mps = simple_cubic_solution(
        coeff_air_drag(), c_rolling_drag, c_rolling_drag * vehicles::rolling_constant_to_variable,
        -total_engine_w);
    add_msg(m_debug, "%s: power %d, c_air %3.2f, c_rolling %3.2f, max_in_mps %3.2f", name,
            total_engine_w, coeff_air_drag(), c_rolling_drag, max_in_mps);
...
int vehicle::max_water_velocity(const bool fueled, const bool ideal) const {
    ...
    add_msg(m_debug, "%s: power %d, c_air %3.2f, c_water %3.2f, water max_in_mps %3.2f", name,
            total_engine_w, coeff_air_drag(), coeff_water_drag(), max_in_mps);
    return mps_to_cmps(max_in_mps);
}
...
int vehicle::safe_ground_velocity(const bool fueled, const bool ideal) const {
    for (size_t e = 0; e < parts.size(); e++) {
        ...
        // Cap safe speed at the point where the slowest animal found would start to damage their
        // yoke If damage or weight has pulled max speed lower than this, cap at that instead.
        if (animal_vel > 0) { return std::min(animal_vel, max_ground_velocity(fueled)); }
    }
    int effective_engine_w = (ideal ? ideal_engine_power(true) : total_power_w(fueled, true));
    double c_rolling_drag = coeff_rolling_drag();
    double safe_in_mps = simple_cubic_solution(
        coeff_air_drag(), c_rolling_drag, c_rolling_drag * vehicles::rolling_constant_to_variable,
        -effective_engine_w);
...
int vehicle::safe_aircraft_velocity(const bool fueled, const bool ideal) const {
    const double max_air_mps = std::sqrt(total_thrust(fueled, true, ideal) / coeff_air_drag());
    return mps_to_cmps(max_air_mps);
}
...
void vehicle::spew_field(double joules, int part, field_type_id type, int intensity) {
    ...
    while (relative_parts.contains(p)) { p.x() += (velocity < 0 ? 1 : -1); }
...
void vehicle::noise_and_smoke(int load, time_duration time) {
    static const std::array<std::pair<std::string, int>, 8> sounds = {
        {{translate_marker("hmm"), 0},
         {translate_marker("hummm!"), 15},
         {translate_marker("whirrr!"), 30},
         {translate_marker("vroom!"), 60},
         {translate_marker("roarrr!"), 100},
         {translate_marker("ROARRR!"), 140},
         {translate_marker("BRRROARRR!"), 180},
         {translate_marker("BRUMBRUMBRUMBRUM!"), INT_MAX}}};
    ...
    for (size_t e = 0; e < engines.size(); e++) {
        ...
        if (is_engine_on(e) && engine_fuel_left(e)) {
            ...
            // Reduce the relative volume of electric engines as there is not literal explosions
            // occouring inside.
            double part_noise =
                cur_stress * part_info(p).engine_noise_factor() * ((electric_engine) ? 0.1 : 1.0);

            if (part_info(p).has_flag("E_COMBUSTION")) {
                combustion = true;
                double health = parts[p].health_percent();
                if (parts[p].base->faults.contains(fault_filter_fuel)) { health = 0.0; }
                if (health < part_info(p).engine_backfire_threshold()
                    && one_in(50 + 150 * health)) {
                    backfire(e);
                }
    ...
    if (is_flying && has_part(VPFLAG_ROTOR)) { noise *= 2; }
    // Speed alone wont generate much noise unless the vehicle is traveling faster than the speed of
    // sound. noise = std::max( noise, std::fabs( velocity / 224.0 ) ); Cap engine noise to avoid
    // deafening. Deafening can occour at or above 140dBspl.
    ...
    if (one_in(4) && rng(0, 30) < noise) {
        while (noise > sounds[lvl].second) { lvl++; }
    }
...
int vehicle::wheel_area() const {
    int total_area = 0;
    for (const int& wheel_index : wheelcache) { total_area += parts[wheel_index].wheel_area(); }

    return total_area;
}

float vehicle::average_or_rating() const {
    if (wheelcache.empty()) { return 0.0f; }
...
double vehicle::coeff_air_drag() const {
    if (!coeff_air_dirty) { return coefficient_air_resistance; }
    ...
    const auto d_exposed = [&](const vehicle_part& p) {
        ...
        if (p.info().location != part_location_center) { return false; }
        return !(
            p.inside || p.info().has_flag("EXTENDABLE") || p.info().has_flag("NO_ROOF_NEEDED")
            || p.info().has_flag("WINDSHIELD") || p.info().has_flag("OPENABLE"));
    };
    ...
    for (int p : structure_indices) {
        ...
        for (int pa_index : parts_at) {
            ...
            d_check_max(
                drag[col].turret, pa,
                pa.info().location == part_location_onroof && !pa.info().has_flag("SOLAR_PANEL")
                    && !pa.info().has_flag("WING"));
            ...
            d_check_min(
                drag[col].last, pa,
                pa.info().has_flag("LOW_FINAL_AIR_DRAG") || pa.info().has_flag("HALF_BOARD"));
        }
    }
    double height = 0;
    double c_air_drag = 0;
    ...
    for (drag_column& dc : drag) {
        ...
        // add_msg( m_debug, "veh %: pro %d, hboard %d, fboard %d, shield %d, seat %d, roof %d,
        // aisle %d, turret %d, panel %d, exposed %d, last %d\n", name, dc.pro, dc.hboard,
        // dc.fboard, dc.shield, dc.seat, dc.roof, dc.aisle, dc.turret, dc.panel, dc.exposed,
        // dc.last );
        ...
        // air drag
        c_air_drag_c +=
            (std::max(std::max(dc.hboard, dc.fboard), dc.shield) != dc.hboard) ? 2 * c_air_mod : 0;
    ...
    if (!balloons.empty()) { c_air_drag += coeff_balloon_drag(); }
...
double vehicle::coeff_balloon_drag() const {
    double volume = std::
        accumulate(balloons.begin(), balloons.end(), double{0.0}, [&](double acc, int balloon) {
...
double vehicle::coeff_rolling_drag() const {
    if (!coeff_rolling_dirty) { return coefficient_rolling_resistance; }
    ...
    coefficient_rolling_resistance =
        newton_ratio * wheel_factor * to_kilogram(total_mass()) * get_lift_percent(true);
...
double vehicle::water_hull_height() const {
    if (coeff_water_dirty) { coeff_water_drag(); }
    return hull_height;
}
...
double vehicle::total_rotor_area() const {
    return std::accumulate(rotors.begin(), rotors.end(), 0.0, [&](double acc, int rotor) {
...
double vehicle::total_propeller_area() const {
    return std::
        accumulate(propellers.begin(), propellers.end(), 0.0, [&](double acc, int propeller) {
...
double vehicle::total_balloon_lift() const {
    return GRAVITY_OF_EARTH
         * std::accumulate(
               balloons.begin(), balloons.end(), double{0.0}, [&](double acc, int balloon) {
                   const double height{parts[balloon].info().balloon_height()};
                   return acc + height;
               });
}
...
double vehicle::total_wing_lift() const {
    ...
    return meterpersecsquared
         * std::accumulate(wings.begin(), wings.end(), double{0.0}, [&](double acc, int wing) {
...
double vehicle::thrust_of_rotorcraft(const bool fuelled, const bool safe, const bool ideal) const {
    ...
    add_msg(m_debug, "lift thrust(N) of %s: %f, rotor area (m^2): %f, engine power (w): %i", name,
            lift_thrust, rotor_area, engine_power);
    return lift_thrust;
}

// constants were converted from imperial to SI goodness
// returns as newton
double vehicle::foward_thrust_of_propellers(
    const bool fuelled, const bool safe, const bool ideal) const {
    constexpr double coefficient = 0.8642;
    constexpr double exponentiation = -0.3107;
...
double vehicle::total_thrust(const bool fuelled, const bool safe, const bool ideal) const {
    return thrust_of_rotorcraft(fuelled, safe, ideal)
         + foward_thrust_of_propellers(fuelled, safe, ideal);
}

// get sum of lift from all lifting parts
double vehicle::total_lift(
    const bool fuelled, const bool safe, const bool ideal, const bool unpowered,
    const bool idle) const {
    if (idle) { return total_balloon_lift(); }
    if (unpowered) {
        return total_balloon_lift() + total_wing_lift();
    } else {
        return thrust_of_rotorcraft(fuelled, safe, ideal) + total_balloon_lift()
             + total_wing_lift();
    }
}

int vehicle::get_takeoff_speed(std::string speed_type) const {
    const int needed_force =
        to_newton(total_mass()) - thrust_of_rotorcraft(true, false, true) - total_balloon_lift();

    const double liftwithoutspeed =
        std::accumulate(wings.begin(), wings.end(), double{0.0}, [&](double acc, int wing) {
            const double liftcoff{parts[wing].info().lift_coff()};
            // m^2 area is always 1
            return acc + (0.5 * liftcoff);
        });
    if (liftwithoutspeed < 1) { return 0; }
    ...
    if (speed_type == "default") { speed_type = get_option<std::string>("USE_METRIC_SPEEDS"); }
    if (speed_type == "km/h") {
...
// For some reason, just checking total lift > 0 doesn't seem to work if the vehicle hasn't been
// piloted before, which was impacting the design view. This fixes it, and can be used to check if
// the vehicle HAS lift, but not enough to fly by doing has_lift && !has_sufficient_lift
...
double vehicle::get_lift_percent(const bool unpowered) const {
    return std::max(0.0, 1 - (total_lift(true, false, false, unpowered) / to_newton(total_mass())));
}

bool vehicle::is_rotorcraft() const { return (has_part(VPFLAG_ROTOR)) && has_sufficient_lift(); }
...
int vehicle::get_z_change() const { return requested_z_change; }

bool vehicle::is_flying_in_air() const { return is_flying; }

void vehicle::set_flying(bool new_flying_value) { is_flying = new_flying_value; }

bool vehicle::is_watercraft() const { return is_floating || (in_water && wheelcache.empty()); }

bool vehicle::is_in_water(bool deep_water) const { return deep_water ? is_floating : in_water; }

static constexpr double water_density = 1000.0; // kg/m^3

double vehicle::coeff_water_drag() const {
    if (!coeff_water_dirty) { return coefficient_water_resistance; }
    ...
    if (hull_indices.empty()) {
        ...
        hull_coverage =
            std::clamp(static_cast<double>(floating.size()) / hull_indices.size(), 0.0, 1.0);
    }

    std::set<int> occupied_y;
    for (int idx : hull_indices) { occupied_y.insert(parts[idx].mount.y()); }
    // Tile == 1m width
...
auto vehicle::vehicle_damage_summary() const -> std::pair<std::string, nc_color> {
    ...
    const int total_damage =
        std::accumulate(vpr.begin(), vpr.end(), 0, [](int lhs, const vpart_reference& rhs) {
...
bool vehicle::is_owned_by(const Character& c, bool available_to_take) const {
    if (owner.is_null()) { return available_to_take; }
    if (!c.get_faction()) {
...
bool vehicle::is_old_owner(const Character& c, bool available_to_take) const {
    if (old_owner.is_null()) { return available_to_take; }
    if (!c.get_faction()) {
...
std::string vehicle::get_owner_name() const {
    if (!g->faction_manager_ptr->get(owner)) {
        debugmsg("vehicle::get_owner_name() vehicle %s has no valid nor null faction id ",
                 disp_name());
        return _("no owner");
    }
...
bool vehicle::has_owner() const { return !owner.is_null(); }

faction_id vehicle::get_owner() const { return owner; }
...
void vehicle::set_owner(const faction_id& new_owner) { owner = new_owner; }

void vehicle::remove_owner() { owner = faction_id::NULL_ID(); }

bool vehicle::has_old_owner() const { return !old_owner.is_null(); }

faction_id vehicle::get_old_owner() const { return old_owner; }
...
bool vehicle::handle_potential_theft(avatar& you, bool check_only, bool prompt) {
    ...
    if (!has_owner()) {
        ...
        remove_old_owner();
        return true;
        // if there is a marker for having been stolen, but 15 minutes have passed, then officially
        // transfer ownership
    }
    ...
    if (!has_witnesses && has_old_owner()) {
        ...
        return true;
        // No witnesses? then don't need to prompt, we assume the player is in process of stealing
        // it. Ownership transfer checking is handled above, and warnings handled below. This is
        // just to perform interaction with the vehicle without a prompt. It will prompt first-time,
        // even with no witnesses, to inform player it is owned by someone else subsequently, no
        // further prompts, the player should know by then.
    }
...
float vehicle::steering_effectiveness() const {
    ...
    if (steering.size() == 1 && vp.info().fuel_type == fuel_type_animal) {
        ...
        if (mon == nullptr || !mon->has_effect(effect_harnessed)) { return -2.0f; }
    }
    ...
    // etc)
    for (int p : steering) {
        if (parts[p].is_available()) { return 1.0f; }
    }
...
std::map<itype_id, int> vehicle::fuel_usage() const {
    ...
    for (size_t i = 0; i < engines.size(); i++) {
        ...
        if (cur_fuel == null_fuel_type) { continue; }

        if (!is_perpetual_type(i)) {
            int usage = info.energy_consumption;
            if (parts[e].faults().contains(fault_filter_air)) { usage *= 2; }
...
double vehicle::drain_energy(const itype_id& ftype, double energy_j) {
    ...
    if (ftype == fuel_type_battery) {
        ...
        if (total_kj_to_drain <= 0) { return 0.0; }
...
void vehicle::consume_fuel(int load, const int t_seconds, bool skip_electric) {
    double st = strain();
    if (current_acceleration() == 0) { return; }
    for (auto& fuel_pr : fuel_usage()) {
        auto& ft = fuel_pr.first;
        if (skip_electric && ft == fuel_type_battery) { continue; }
    ...
    if (load > 0 && fuel_left(fuel_type_muscle) > 0) {
        ...
        int mod = 4 * st; // strain
        int base_burn = static_cast<int>(get_option<float>("PLAYER_BASE_STAMINA_REGEN_RATE")) - 3;
        ...
        for (vpart_reference vp : get_enabled_parts(VPFLAG_ENGINE)) {
            if (vp.info().fuel_type == fuel_type_muscle) {
                ...
                if (p
                    && ((vp.info().has_flag("MUSCLE_LEGS") && (p->get_working_leg_count() >= 2))
                        || (vp.info().has_flag("MUSCLE_ARMS")
                            && (p->get_working_arm_count() >= 2)))) {
                    ...
                    for (const bionic_id& bid : p->get_bionic_fueled_with(muscle)) {
                        if (p->has_active_bionic(bid)) { // active power gen
                            ...
                            p->mod_power_level(
                                units::from_kilojoule(muscle.fuel_energy()) * bid->fuel_efficiency
                                * (load / 1000.0f));
                            mod += eff_load / 5;
                        } else { // passive power gen
                            p->mod_power_level(
                                units::from_kilojoule(muscle.fuel_energy())
                                * bid->passive_fuel_efficiency * (load / 1000.0f));
                            mod += eff_load / 10;
                        }
                    }
                    ...
                    if (p->has_active_bionic(bio_jointservo)) {
                        p->mod_power_level(
                            -bio_jointservo->power_trigger * std::max(eff_load / 20, 1));
...
std::vector<vehicle_part*> vehicle::lights(bool active) {
    std::vector<vehicle_part*> res;
    for (auto& e : parts) {
        if ((!active || e.enabled) && e.is_available() && e.is_light()) { res.push_back(&e); }
    }
    return res;
}
...
int vehicle::total_alternator_epower_w() const {
    ...
    if (engine_on) {
        ...
        for (size_t p = 0; p < alternators.size(); ++p) {
            if (is_alternator_on(p)) { epower += part_epower_w(alternators[p]); }
        }
    }
    return epower;
}
...
int vehicle::total_engine_epower_w() const {
    ...
    if (engine_on) {
        for (size_t e = 0; e < engines.size(); ++e) {
            if (is_engine_on(e)) { epower += part_epower_w(engines[e]); }
        }
    }
...
int vehicle::total_solar_epower_w() const {
    ...
    for (int part : solar_panels) {
        if (parts[part].is_unavailable()) { continue; }

        if (!is_sm_tile_outside(abs_part_location(part))) { continue; }
...
int vehicle::total_wind_epower_w() const {
    ...
    const oter_id& cur_om_ter =
        get_overmapbuffer(dimension_id_).ter(project_to<coords::omt>(abs_ms_location()));
    const weather_manager& weather = get_weather();
    int epower_w = 0;
    for (int part : wind_turbines) {
        ...
        double windpower = get_local_windpower(
            weather.windspeed, cur_om_ter, abs_part_location(part), weather.winddirection, false);
        if (windpower <= (weather.windspeed / 10.0)) { continue; }
...
int vehicle::total_water_wheel_epower_w() const {
    ...
    for (int part : water_wheels) {
        ...
        if (!is_sm_tile_over_water(abs_part_location(part))) { continue; }
...
int vehicle::max_reactor_epower_w() const {
    int epower_w = 0;
    for (int elem : reactors) { epower_w += is_part_on(elem) ? part_epower_w(elem) : 0; }
    return epower_w;
}
...
void vehicle::update_alternator_load() {
    ...
    if (engine_on) {
        ...
        for (size_t p = 0; p < alternators.size(); ++p) {
            if (is_alternator_on(p)) { alternators_power += part_vpower_w(alternators[p]); }
        }
        alternator_load =
            engine_vpower
...
void vehicle::power_parts(int turns) {
    ...
    int epower =
        engine_epower + ((total_accessory_epower_w() + total_alternator_epower_w()) * turns);

    int delta_energy_bat = power_to_energy_bat(epower, 1_turns);
    int storage_deficit_bat = std::
        max(0, fuel_capacity(fuel_type_battery) - fuel_left(fuel_type_battery) - delta_energy_bat);
    ...
    if (!reactors.empty() && storage_deficit_bat > 0) {
        ...
        if (!reactor_working && reactor_online) {
            ...
            for (auto& elem : reactors) { parts[elem].enabled = false; }
    ...
    if (battery_deficit != 0) {
        ...
        for (const vpart_reference& vp : get_enabled_parts("SCOOP")) { vp.part().enabled = false; }
        ...
        for (const vpart_reference& vp : get_enabled_parts(VPFLAG_ENABLED_DRAINS_EPOWER)) {
            vehicle_part& pt = vp.part();
            if (pt.info().epower < 0) { pt.enabled = false; }
        }
...
vehicle* vehicle::find_vehicle(const tripoint_abs_ms& where, mapbuffer& mbuf) {
    ...
    auto veh_local = abs_to_bub(where);
    if (const optional_vpart_position vp = g->m.veh_at(veh_local)) { return &vp->vehicle(); }
    ...
    for (auto& elem : sm->vehicles) {
        ...
        if (proj.remainder == found_veh->sm_ms_pos) { return found_veh; }
    }

    return nullptr;
}

void vehicle::enumerate_vehicles(
    ...
    for (vehicle* veh : vehicle_list) {
        ...
        distribution_graph::
            traverse(*veh, enumerate_visitor, distribution_graph::noop_visitor_grid);
    }
}
...
template <bool IsConst, typename Vehicle = std::conditional_t<IsConst, const vehicle, vehicle>,
          typename Grid = std::conditional_t<IsConst, const distribution_grid, distribution_grid>>
struct vehicle_or_grid {
    enum class type_t : char { vehicle, grid } type;

    Vehicle* veh = nullptr;
    Grid* grid = nullptr;

    vehicle_or_grid(Vehicle* veh): type(type_t::vehicle), veh(veh) {}

    vehicle_or_grid(Grid* grid): type(type_t::grid), grid(grid) {}

    bool operator==(const vehicle_or_grid& other) const {
        return veh == other.veh && grid == other.grid;
    }

    bool operator==(const vehicle* veh) const { return this->veh == veh; }

    bool operator==(const distribution_grid* grid) const { return this->grid == grid; }
};

template <typename VehFunc, typename GridFunc, typename StartPoint>
void traverse(StartPoint& start, VehFunc veh_action, GridFunc grid_action) {
    ...
    struct hash {
        auto operator()(const vehicle_or_grid<IsConst>& elem) const {
            return std::hash<char>{}(static_cast<char>(elem.type))
                 ^ std::hash<size_t>{}(
                       // Because only one of pointers is not nullptr, binary OR would get value of
                       // set pointer.
    ...
    auto enqueue =
        [&connected_elements, &visited_elements](vehicle_or_grid<IsConst> newly_visited) {
    ...
    auto process_vehicle = [&](const tripoint_abs_ms& target_pos) {
        ...
        if (was_already_visited(veh)) { return tvr::continue_further; }

        const tvr result = veh_action(*veh);
        g->u.add_msg_if_player(m_debug, "After remote veh %p", static_cast<void*>(veh));

        // We do not need to check neighbours if we stop.
        if (result == tvr::continue_further) { enqueue(veh); }
    ...
    auto process_grid = [&](const tripoint_abs_ms& target_pos) {
        ...
        if (was_already_visited(&grid)) { return tvr::continue_further; }

        const tvr result = grid_action(grid);
        g->u.add_msg_if_player(m_debug, "After remote grid %p", static_cast<void*>(&grid));

        // We do not need to check neighbours if we stop.
        if (result == tvr::continue_further) { enqueue(&grid); }
        ...
        if (current.type == vehicle_or_grid<IsConst>::type_t::vehicle) {
            ...
            for (auto& p : current_veh.loose_parts) {
                ...
                if (current_veh.cpart(p).has_flag(vehicle_part::targets_grid)) {
                    if (process_grid(target_pos) == tvr::stop) { return; }
                } else {
                    if (process_vehicle(target_pos) == tvr::stop) { return; }
                }
            }
        } else {
            ...
            for (auto& p : current_grid.get_contents()) {
                const vehicle_connector_tile* connector =
                    active_tiles::furn_at<vehicle_connector_tile>(p);
...
int vehicle::charge_battery(int amount, bool include_other_vehicles) {
    ...
    while (amount > 0) {
        ...
        if (lowest_charge_level >= 100) { break; }
...
int vehicle::discharge_battery(int amount, bool recurse) {
    ...
    while (amount > 0) {
        ...
        if (dischargeable_parts[highest_charge_level].empty()) { break; }
        ...
        const auto amount_to_discharge =
            std::min(part.ammo_remaining() - prev_charge_level, amount);
...
void vehicle::do_engine_damage(size_t e, int strain) {
    ...
    if (is_engine_on(e) && !is_perpetual_type(e) && engine_fuel_left(e) && rng(1, 100) < strain) {
...
void vehicle::idle_turns(const int turns) {
    ...
    if (engine_on && total_power_w() > 0) {
        ...
        /*
            TODO: Consider different formula for idling aircraft, may need a formula to determine
           this Possibly something like total lift / total engine power, maybe some factors for
           hovering efficiency of different types Also consider adding a hover efficiency field
        */
        if (is_rotorcraft() && is_flying_in_air()) {
            const auto rotor_newtons =
                std::max(0.0, to_newton(total_mass()) - total_balloon_lift() - total_wing_lift());
        ...
        if (engine_on && g->u.sees(bub_ms_location())
            && (has_engine_type_not(fuel_type_muscle, true)
                ...
                && has_engine_type_not(fuel_type_mana, true))) {
            add_msg(_("The %s's engine dies!"), name);
        }
        engine_on = false;
    }
...
void vehicle::idle(bool on_map) {
    ...
    if (engine_on && total_power_w() > 0) {
        ...
        */
        ...
        if (on_map) { noise_and_smoke(idle_rate, 1_turns); }
    } else {
        if (engine_on && g->u.sees(bub_ms_location())
            && (has_engine_type_not(fuel_type_muscle, true)
                && has_engine_type_not(fuel_type_animal, true)
    ...
    if (has_part("STEREO", true)) { play_music(); }

    if (has_part("CHIMES", true)) { play_chimes(); }

    if (has_part("CRASH_TERRAIN_AROUND", true)) { crash_terrain_around(); }

    if (is_alarm_on) { alarm(); }

    // V-3: skip the full part scan when no AUTOLOADER parts are installed.
    if (has_autoloaders) { vehicle_funcs::process_autoloaders(*this); }
}

void vehicle::on_move() {
    if (has_part("TRANSFORM_TERRAIN", true)) { transform_terrain(); }
    if (has_part("SCOOP", true)) { operate_scoop(); }
    if (has_part("PLANTER", true)) { operate_planter(); }
    if (has_part("REAPER", true)) { operate_reaper(); }
...
void vehicle::slow_leak() {
    ...
    for (auto& p : parts) {
        ...
        if (!p.is_leaking() || p.ammo_remaining() <= 0) { continue; }
        ...
        if (fuel != fuel_type_battery && fuel != fuel_type_plutonium_cell) {
            ...
            if (p.ammo_remaining() >= PLUTONIUM_CHARGES / 10) {
                g->m.add_item_or_charges(
                    dest, item::spawn("plut_slurry_dense", calendar::turn, qty));
...
units::volume vehicle::max_volume(const int part) const { return get_items(part).max_volume(); }

units::volume vehicle::free_volume(const int part) const { return get_items(part).free_volume(); }

void vehicle::make_inactive(item& target) {
    auto cargo_parts = get_parts_at(target.bub_pos(), "CARGO", part_status_flag::any);
...
auto vehicle::invalidate_cargo_recharge_cache() -> void { cargo_recharge_targets_dirty = true; }
        ...
        for (const vpart_reference& vp : get_parts_including_carried(VPFLAG_CARGO)) {
            for (item*& outer : get_items(static_cast<int>(vp.part_index()))) {
                outer->visit_items([this, &vp](item* it) {
                    if (!is_cargo_recharge_candidate(*it)) { return VisitResponse::NEXT; }
...
detached_ptr<item> vehicle::add_item(int part, detached_ptr<item>&& itm) {
    ...
    // const int max_weight = ?! // TODO: weight limit, calculation per vpart & vehicle stats, not a
    // hard user limit. add creaking sounds and damage to overloaded vpart, outright break it past a
    // certain point, or when hitting bumps etc
    vehicle_part& p = parts[part];
    if (p.is_broken()) { return std::move(itm); }
    ...
    if (itm->is_bucket_nonempty()) { itm->contents.spill_contents(bub_part_location(part)); }
    if (itm->needs_processing()) { active_items.add(*itm); }
...
detached_ptr<item> vehicle::remove_item(int part, item* it) {
    ...
    const location_vector<item>::const_iterator iter =
        std::find_if(veh_items.begin(), veh_items.end(), [&it](const item* const& item) {
            return it == item;
        });

    if (iter == veh_items.end()) { return detached_ptr<item>(); }
    detached_ptr<item> det;
    remove_item(part, iter, &det);
    return det;
}

vehicle_stack::iterator vehicle::remove_item(
    int part, vehicle_stack::const_iterator it, detached_ptr<item>* ret) {
...
void vehicle::place_spawn_items() {
    ...
    std::ranges::for_each(type->parts, [this](const auto& pt) {
        ...
        if (x_in_y(scaled_chance, 100)) {
            parts[turret]
                .ammo_set(random_entry(pt.ammo_types), rng(pt.ammo_qty.first, pt.ammo_qty.second));
        }
    });

    std::ranges::for_each(type.obj().item_spawns, [this](const auto& spawn) {
        if (rng(1, 100) > spawn.chance) { return; }
        ...
        if (broken && one_in(2)) { return; }

        std::vector<detached_ptr<item>> created;
        created.reserve(spawn.item_ids.size());
        std::ranges::transform(spawn.item_ids, std::back_inserter(created), [](const itype_id& e) {
        ...
        std::erase_if(created, [broken, global_spawn_rate](detached_ptr<item>& e) {
            ...
            if (broken && e->mod_damage(rng(1, e->max_damage()))) { return true; }
        ...
        std::ranges::for_each(created, [this, &spawn, part](detached_ptr<item>& e) {
            if (e->is_tool() || e->is_gun() || e->is_magazine()) {
                ...
                auto spawn_mag =
                    rng(0, 99) < spawn.with_magazine && !e->magazine_integral()
                    && !e->magazine_current();

                if (spawn_mag) { e->put_in(item::spawn(e->magazine_default(), e->birthday())); }
                if (spawn_ammo) { e->ammo_set(e->ammo_default()); }
            }
...
void vehicle::gain_moves() {
    ...
    distribution_graph::traverse(
        *this, distribution_graph::noop_visitor_veh, distribution_graph::noop_visitor_grid);

    if (check_environmental_effects) { check_environmental_effects = do_environmental_effects(); }
    ...
    for (auto e : turrets()) {
        if (e->enabled || e->target.second != e->target.first) { automatic_fire_turret(*e); }
    }

    if (velocity < 0) { beeper_sound(); }
}
...
bool vehicle::decrement_summon_timer() {
    if (!summon_time_limit) { return false; }
...
/**
 * Refreshes all caches and refinds all parts. Used after the vehicle has had a part added or
 * removed. Makes indices of different part types so they're easy to find. Also calculates power
 * drain.
 */
void vehicle::refresh() {
    ...
    for (const vpart_reference& vp : get_all_parts()) {
        ...
        std::vector<int>::iterator vii = std::lower_bound(
            relative_parts[pt].begin(), relative_parts[pt].end(), static_cast<int>(p), svpv);
        relative_parts[pt].insert(vii, p);

        if (vpi.has_flag(VPFLAG_FLOATS)) { floating.push_back(p); }
        if (vp.part().is_battery()) { battery_parts.push_back(p); }

        if (vp.part().is_unavailable()) { continue; }
        ...
        if (vpi.has_flag(VPFLAG_ROTOR)) { rotors.push_back(p); }
        if (vpi.has_flag(VPFLAG_PROPELLER)) { propellers.push_back(p); }
        if (vpi.has_flag(VPFLAG_WING)) { wings.push_back(p); }
        if (vpi.has_flag(VPFLAG_BALLOON)) { balloons.push_back(p); }
        if (vpi.has_flag("CONVERTER")) { converters.push_back(p); }
        if (vpi.has_flag("FLUIDTANK")) { tanks.push_back(p); }
        if (vpi.has_flag(VPFLAG_DROPPER)) { droppers.push_back(p); }
        if (vpi.has_flag("WIND_TURBINE")) { wind_turbines.push_back(p); }
        if (vpi.has_flag("WIND_POWERED")) { sails.push_back(p); }
        if (vpi.has_flag("WATER_WHEEL")) { water_wheels.push_back(p); }
        if (vpi.has_flag("FUNNEL")) { funnels.push_back(p); }
        if (vpi.has_flag("UNMOUNT_ON_MOVE")) { loose_parts.push_back(p); }
        if (vpi.has_flag("EMITTER")) { emitters.push_back(p); }
        if (vpi.has_flag("AUTOLOADER")) { has_autoloaders = true; }
        if (vpi.has_flag(VPFLAG_RECHARGE)) { has_cargo_recharge = true; }
        if (vpi.has_flag(VPFLAG_WHEEL)) { wheelcache.push_back(p); }
        if (vpi.has_flag(VPFLAG_RAIL)) {
            ...
            if (it == rail_profile.cend()) { rail_profile.push_back(rail_pos); }
        }
        ...
        if (vpi.has_flag("SECURITY")) { speciality.push_back(p); }
        if (vp.part().enabled && vpi.has_flag("EXTRA_DRAG")) { extra_drag += vpi.power; }
        if (vpi.has_flag("EXTRA_DRAG")
            && (vpi.has_flag("WIND_TURBINE") || vpi.has_flag("WATER_WHEEL"))) {
            extra_drag += vpi.power;
        }
        if (vpi.has_flag("POWERED_BY_ENGINE")) { extra_drag += vpi.power; }
    ...
    if (!refresh_done) { mount_min = mount_max = tripoint_mnt_veh::zero(); }
...
void vehicle::refresh_pivot() const {
    ...
    if (xc_denominator < 0.1 || yc_denominator < 0.1) {
        debugmsg("vehicle::refresh_pivot had a bad weight: xc=%.3f/%.3f yc=%.3f/%.3f", xc_numerator,
                 xc_denominator, yc_numerator, yc_denominator);
        pivot_cache = local_center_of_mass();
    } else {
...
void vehicle::do_towing_move() {
    if (!no_towing_slack() || velocity <= 0) { return; }
    ...
    if (rl_dist(towed_tow_point, tower_tow_point) < 6) { accel_y = reverse ? -1 : 1; }
    ...
    if (rl_dist(towed_tow_point, tower_tow_point) >= 12) {
        ...
        if (reverse) { towed_veh->velocity = -towed_veh->velocity; }
    } else {
    ...
    if (towed_veh->tow_data.tow_direction == TOW_FRONT) {
        ...
        std::vector<tripoint_bub_ms> lineto = line_to(abs_to_bub(towed_tow_point), tower_tow_local);
        ...
        const tripoint_rel_ms move_destination(
            clamp(destination_delta_x, -1, 1), clamp(destination_delta_y, -1, 1),
...
bool vehicle::is_external_part(const tripoint_bub_ms& part_pt) const {
    for (const auto& elem : g->m.points_in_radius(part_pt, 1)) {
        ...
        if (!vp) { return true; }
        if (vp && &vp->vehicle() != this) { return true; }
    }
    return false;
}
...
bool vehicle::is_towed() const {
    ...
    if (!tow_data.get_towed_by()) {
        ...
        if (!tow_data.get_towed_by()->tow_data.get_towed()) {
            debugmsg("vehicle %s is marked as towed, but the tower vehicle has no towed defined",
                     name);
            return ret;
        }
        ret = true;
...
int vehicle::get_tow_part() const {
    for (const vpart_reference& vp : get_all_parts()) {
        ...
        if (part_with_feature(p, "TOW_CABLE", true) >= 0 && vp.part().is_available()) { return p; }
    }
    return -1;
}
...
void vehicle::set_tow_directions() {
    ...
    // Fairly certain including the z axis here does nothing for now, but might be useful in the
    // future
    const auto normalized_tow_mount = tripoint_mnt_veh(
        std::abs(mount_of_tow.x() - mount_min.x()), std::abs(mount_of_tow.y() - mount_min.y()),
...
bool towing_data::set_towing(vehicle* tower_veh, vehicle* towed_veh) {
    if (!towed_veh || !tower_veh) { return false; }
...
void vehicle::invalidate_towing(bool first_vehicle) {
    ...
    if (other_veh && first_vehicle) { other_veh->invalidate_towing(); }
    for (const vpart_reference& vp : get_all_parts()) {
        const size_t p = vp.part_index();
        if (vp.part().removed) { continue; }
...
bool vehicle::tow_cable_too_far() const {
    ...
    if (!tow_data.get_towed_by()->tow_data.get_towed()) {
        debugmsg(
            "vehicle %s has data for a towing vehicle, but that towing vehicle does not have %s listed as towed",
            disp_name(), disp_name());
        return false;
    }
...
bool vehicle::no_towing_slack() const {
    if (!tow_data.get_towed()) { return false; }
    int index = get_tow_part();
    if (index == -1) {
        debugmsg("towing data exists but no towing part");
        return false;
    ...
    if (!tow_data.get_towed()->tow_data.get_towed_by()) {
        debugmsg(
            "vehicle %s has data for a towed vehicle, but that towed vehicle does not have %s listed as tower",
            disp_name(), disp_name());
        return false;
    }
...
void vehicle::remove_remote_part(int part_num) {
    if (parts[part_num].has_flag(vehicle_part::targets_grid)) {
        vehicle_connector_tile* connector = active_tiles::furn_at<vehicle_connector_tile>(
            parts[part_num].target.second);
        if (connector != nullptr) {
            auto& vehs = connector->connected_vehicles;
            const auto iter = std::ranges::find(vehs, abs_ms_location());
            if (iter != vehs.end()) { vehs.erase(iter); }
        }
        return;
    }
    ...
    if (veh != nullptr) {
        ...
        for (size_t j = 0; j < veh->loose_parts.size(); j++) {
            ...
            if (veh->part_flag(remote_partnum, "POWER_TRANSFER")
...
void vehicle::shed_loose_parts() {
    ...
    while (!loose_parts.empty()) {
        ...
        if (part_flag(elem, "POWER_TRANSFER")) { remove_remote_part(elem); }
...
bool vehicle::enclosed_at(const tripoint_bub_ms& pos) {
    refresh_insides();
    std::vector<vehicle_part*> parts_here =
        get_parts_at(pos, "BOARDABLE", part_status_flag::working);
    if (!parts_here.empty()) { return parts_here.front()->inside; }
    return false;
}

void vehicle::refresh_insides() {
    if (!insides_dirty) { return; }
    insides_dirty = false;
    for (const vpart_reference& vp : get_all_parts()) {
        const size_t p = vp.part_index();
        if (vp.part().removed) { continue; }
...
void vehicle::unboard_all() {
    ...
    for (auto& i : bp) { g->m.unboard_vehicle(bub_part_location(i)); }
}
...
int vehicle::damage(int p, int dmg, damage_type type, bool aimed, bool random_part) {
    ...
    if (!aimed) {
        ...
        for (auto& i : pl) {
            if (part_flag(i, "OBSTACLE") && (!part_flag(i, "OPENABLE") || !parts[i].open)) {
                found_obs = true;
                break;
            }
        }
    ...
    int target_part = [&]() {
        if (random_part) {
            if (part_info(p).rotor_diameter() && one_in(2)) { return p; }
    ...
    if (part_flag(target_part, "DOOR_MOTOR")) {
        ...
        if (strongest_door_part != -1) { target_part = strongest_door_part; }
    }
    ...
    if (armor_part < 0) {
        ...
        bool overhead =
            part_flag(target_part, "ROOF") || part_info(target_part).location == "on_roof";
        ...
        if (distance > 1) {
            ...
            if (shockpart.location != part_location_structure
                ...
                if (shock_absorber >= 0) {
                    net_dmg = std::max(
                        0.0f,
                        net_dmg - (parts[shock_absorber].info().bonus)
                ...
                if (shockpart.has_flag("SHOCK_RESISTANT")) {
                    ...
                    for (const int elem : all_parts_at_location(shockpart.location)) {
                        // Original intent was to find the frame that the part was mounted on and
                        // grab that objects resistance, but instead we will go with half the
                        // largest damage resist in the stack.
                        damage_resist = std::
                            max(damage_resist, part_info(elem).damage_reduction.type_resist(type));
                    }
                    damage_resist = damage_resist / 2;

                    add_msg(m_debug, "%1s inherited %.1f damage resistance!", shockpart.name(),
                            damage_resist);
            ...
            if (net_dmg > part_info(p).damage_reduction.type_resist(type)) {
                ...
                add_msg(m_debug, _("%1s took %.1f damage from shock."), part_info(p).name(),
                        1.0f * net_dmg);
            }
        }
    }
...
void vehicle::shift_parts(const tripoint_rel_veh& delta) {
    for (auto& elem : parts) { elem.mount -= delta; }

    decltype(labels) new_labels;
    for (auto& l : labels) { new_labels.insert(label(l - delta, l.text)); }
    labels = new_labels;

    decltype(loot_zones) new_zones;
    for (auto const& z : loot_zones) { new_zones.emplace(z.first - delta, z.second); }
...
bool vehicle::shift_if_needed() {
    ...
    for (const vpart_reference& vp : get_all_parts()) {
        if (vp.info().location == "structure" && !vp.has_feature("PROTRUSION")
            && !vp.part().removed) {
...
int vehicle::break_off(int p, int dmg) {
    ...
    if (rng(0, part_info(p).durability / 10) >= dmg) { return dmg; }
    ...
    const auto scatter_parts = [&](const vehicle_part& pt) {
        for (detached_ptr<item>& piece : pt.pieces_for_broken_part()) {
            ...
            if (!magic) { g->m.add_item_or_charges(where, std::move(piece)); }
        }
    };
    if (part_info(p).location == part_location_structure) {
        ...
        for (int index = parts_in_square.size() - 1; index >= 0; index--) {
            ...
            if (parts_in_square[index] == p) { continue; }
            ...
            if (parts[parts_in_square[index]].is_broken()) {
                ...
                if (!magic) {
                    g->m.add_item_or_charges(
                        pos, parts[parts_in_square[index]].properties_to_item());
                }
            }
...
bool vehicle::explode_fuel(int p, damage_type type) {
    ...
    if (!fuel.has_explosion_data()) { return false; }
    fuel_explosion data = fuel.get_explosion_data();

    if (parts[p].is_broken()) { leak_fuel(parts[p]); }
    ...
    if (one_in(explosion_chance)) {
        ...
        const int pow =
            120
            * (1
               - std::exp(data.explosion_factor / -5000
                          * (parts[p].ammo_remaining() * data.fuel_size_factor)));
        // debugmsg( "damage check dmg=%d pow=%d amount=%d", dmg, pow, parts[p].amount );

        explosion_handler::explosion(bub_part_location(p), nullptr, pow, 0.7, data.fiery_explosion);
...
unsigned int vehicle::hits_to_destroy(int p, int dmg, damage_type type) const {
    ...
    const bool is_armor_considered = !(
        armor_part < 0 || part_flag(p, VPFLAG_ROOF) || part_info(p).location == "on_roof");
    ...
    if (part_dmg_without_armor <= 0
        || (type != DT_TRUE && part_dmg_without_armor < part_threshold_damage)) {
        return 0;
    }
    ...
    if (!is_armor_considered) {
        const int part_htd =
            part_hp / part_dmg_without_armor + (part_hp % part_dmg_without_armor > 0);
        return part_htd;
    }
    ...
    const int armor_htd =
        armor_dmg <= 0 || (type != DT_TRUE && armor_dmg < armor_threshold_damage)
            ? INT_MAX
    ...
    const int part_htd_with_armor =
        (part_dmg_with_armor <= 0
    ...
    if (part_htd_with_armor <= armor_htd) { return part_htd_with_armor; }

    // Armor gets destroyed before part does
    const int part_hp_after_armor = part_hp - armor_htd * std::max(part_dmg_with_armor, 0);
    const int part_htd_after_armor =
        part_hp_after_armor / part_dmg_without_armor
...
int vehicle::damage_direct(int p, int dmg, damage_type type) {
    ...
    if (dmg < tsh && type != DT_TRUE) {
        if (type == DT_HEAT && parts[p].is_fuel_store()) { explode_fuel(p, type); }
...
void vehicle::leak_fuel(vehicle_part& pt) {
    ...
    if (!pt.is_tank() || pt.ammo_remaining() <= 0) { return; }
    ...
    tiles.erase(
        std::remove_if(
            tiles.begin(), tiles.end(), [](const tripoint_bub_ms& e) { return !g->m.passable(e); }),
        tiles.end());
    ...
    while (!tiles.empty() && pt.ammo_remaining()) {
        int qty =
            pt.ammo_consume(rng(0, std::max(pt.ammo_remaining() / 3, 1)), bub_part_location(pt));
        if (qty > 0) {
...
bool vehicle::is_foldable() const {
    for (const vpart_reference& vp : get_all_parts()) {
        if (!vp.has_feature("FOLDABLE")) { return false; }
    }
    return true;
}
...
bool vehicle::restore(const std::string& data) {
    ...
    for (vehicle_part& part : parts) { part.hack_id = get_next_hack_id(); }
    refresh_locations_hack();
    refresh();
    face.init(0_degrees);
    turn_dir = 0_degrees;
...
std::set<tripoint_abs_ms>& vehicle::get_points(const bool force_refresh) {
    if (force_refresh || occupied_cache_time != calendar::turn) {
        ...
        for (const auto& p : parts) { occupied_points.insert(abs_part_location(p)); }
    }
...
const vpart_info& vpart_reference::info() const { return part().info(); }

player* vpart_reference::get_passenger() const { return vehicle().get_passenger(part_index()); }

tripoint_mnt_veh vpart_position::mount() const { return vehicle().part(part_index()).mount; }

tripoint_bub_ms vpart_position::pos() const { return vehicle().bub_part_location(part_index()); }

bool vpart_reference::has_feature(const std::string& f) const { return info().has_flag(f); }

bool vpart_reference::has_feature(const vpart_bitflags f) const { return info().has_flag(f); }
...
static bool is_sm_tile_over_water(const tripoint_abs_ms& pos) {
    ...
    if (sm == nullptr) {
        debugmsg("is_sm_tile_over_water(): couldn't find submap %d,%d,%d",
                 proj.quotient_tripoint.x(), proj.quotient_tripoint.y(),
                 proj.quotient_tripoint.z());
        return false;
    }
...
void vehicle::update_time(const time_point& update_to, const bool batched) {
    ...
    if (!converters.empty()) {
        for (int p : converters) {
            ...
            if (!part.is_unavailable() && part.enabled) {
                ...
                const item* output =
                    item::spawn_temporary(output_type, calendar::turn, output_charges);
                ...
                if (!consume_type.is_null()) {
                    auto consume_tank_idx = std::ranges::find_if(tanks, [&](int tank) {
                        ...
                        if (part.ammo_current() == consume_type
                            && part.ammo_remaining() > consume_charges) {
                            return true;
                        }
                        return false;
                    });
                    if (consume_tank_idx == tanks.end()) { continue; }
                ...
                if (!output_type.is_null()) {
                    auto output_tank_idx = std::ranges::find_if(tanks, [&](int tank) {
                        const auto& part = parts[tank];
                        if (part.can_reload(output)
                            && part.ammo_capacity() - part.ammo_remaining() > output_charges) {
                            return true;
                        }
                        return false;
                    });
                    if (output_tank_idx == tanks.end()) { continue; }
                    output_tank = &parts[*output_tank_idx];
                }
                int max_repeats = repeat;
                if (consume_tank) {
                    max_repeats =
                        std::min(max_repeats, consume_tank->ammo_remaining() / consume_charges);
                }
                if (output_tank) {
                    max_repeats = std::min(
                        max_repeats,
                        (output_tank->ammo_capacity() - output_tank->ammo_remaining())
                            / output_charges);
                }
                ...
                if (consume_tank != nullptr) {
                    consume_tank->ammo_consume(
                        max_repeats * consume_charges, bub_part_location(*consume_tank));
                }
                if (output_tank != nullptr) {
                    output_tank->ammo_set(
                        output_type, output_tank->ammo_remaining() + max_repeats * output_charges);
                }
    ...
    const weather_sum accum_weather = sum_conditions(update_from, update_to, abs_ms_location());
    ...
    for (int idx : funnels) {
        ...
        if (pt.is_unavailable() || !is_sm_tile_outside(abs_part_location(pt))) { continue; }
    ...
    if (!solar_panels.empty()) {
        ...
        double intensity =
            accum_weather.sunlight / default_daylight_level() / to_turns<double>(elapsed);
...
void vehicle::process_emitters() {
    ...
    for (int idx : emitters) {
        ...
        if (pt.is_unavailable() || !pt.enabled) { continue; }
        for (const emit_id& e : pt.info().emissions) { g->m.emit_field(bub_part_location(pt), e); }
    }
}
...
void vehicle::refresh_mass() const { calc_mass_center(true); }
...
void vehicle::calc_mass_center(bool use_precalc) const {
    ...
    for (const vpart_reference& vp : get_all_parts()) {
        ...
        if (weight_modifier != 100) { m_part *= static_cast<float>(weight_modifier) / 100.0f; }
        for (const auto& j : get_items(i)) { m_part_items += j->weight(); }
...
int vehicle::part_count() const { return static_cast<int>(parts.size()); }

vehicle_part& vehicle::part(int part_num) { return parts[part_num]; }
...
std::set<int> vehicle::advance_precalc_mounts(const tripoint_abs_ms& src) {
    ...
    for (vehicle_part& prt : parts) {
        here.clear_vehicle_point_from_cache(
            ...
            src_local
                + tripoint_rel_ms(
                    prt.precalc[0].x(), prt.precalc[0].y(), prt.mount.z() + prt.z_terrain[0]));
...
bool vehicle::refresh_zones() {
    if (zones_dirty) {
        ...
        for (auto const& z : loot_zones) {
            ...
            if (part_idx == -1) {
                debugmsg(
                    "Could not find cargo part at %d,%d on vehicle %s for loot zone.  Removing loot zone.",
                    z.first.x(), z.first.y(), this->name);

                // If this loot zone refers to a part that no longer exists at this location, then
                // its unattached somehow. By continuing here and not adding to new_zones, we
...
template <> bool vehicle_part_with_feature_range<std::string>::matches(const size_t part) const {
    const vehicle_part& vp = this->vehicle().part(part);
    return vp.info().has_flag(feature_) && !vp.removed
...
template <> bool vehicle_part_with_feature_range<vpart_bitflags>::matches(const size_t part) const {
    const vehicle_part& vp = this->vehicle().part(part);
    return vp.info().has_flag(feature_) && !vp.removed
        && (!(part_status_flag::working & required_) || !vp.is_broken())
        && (!(part_status_flag::available & required_) || vp.is_available())
...
void vehicle::refresh_locations_hack() {
    for (vehicle_part& part : parts) { part.refresh_locations_hack(this); }
}

const vehicle_part* vehicle::find_part_hack(const int id) const {
    for (const vehicle_part& part : parts) {
        if (part.hack_id == id) { return &part; }
    }
    return nullptr;
}

vehicle_part* vehicle::find_part_hack(const int id) {
    return const_cast<vehicle_part*>(static_cast<const vehicle*>(this)->find_part_hack(id));
}

int vehicle::get_part_id_hack(const int id) const {
    int i = 0;
    for (const vehicle_part& part : parts) {
        if (part.hack_id == id) { return i; }
        i++;
    }
    return -1;
}
...
void vehicle::item_dropper_drop(std::vector<vehicle_part*> droppers, bool single) {
    if (single) {
        ...
        while (here.has_flag_ter_or_furn(TFLAG_NO_FLOOR, pos)) { pos.z() -= 1; }
        item* dropper = options[idx];
        if (dropper->get_use("transform")) { g->u.invoke_item(dropper, "transform"); }
        ...
        for (vehicle_part* d : droppers) {
            ...
            if (part) {
                ...
                for (item* it : items) {
                    if (it->get_use("transform")) { g->u.invoke_item(it, "transform"); }
...
void vehicle::item_dropper_drop_all() {
    ...
    for (int idx : droppers) {
        vehicle_part* d = &parts[idx];
        if (has_item_stored(d)) { ret.push_back(d); }
    }

    item_dropper_drop(ret, false);
}
void vehicle::set_cruise_control_speed() {
    const auto requested_min_autodrive_speed =
        string_input_popup()
            ...
            .query_int();
    min_autodrive_speed =
        requested_min_autodrive_speed > 0 ? requested_min_autodrive_speed : min_autodrive_speed;
    const auto requested_max_autodrive_speed =
        string_input_popup()
            ...
            .query_int();
    max_autodrive_speed =
        requested_max_autodrive_speed > 0 ? requested_max_autodrive_speed : max_autodrive_speed;
    if (max_autodrive_speed < min_autodrive_speed) { max_autodrive_speed = min_autodrive_speed; }

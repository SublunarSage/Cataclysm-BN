...
vehicle_part& most_repairable_part(vehicle& veh, Character& who, bool only_repairable) {
    ...
    for (const vpart_reference& vpr : veh.get_all_parts()) {
        ...
        if (info.is_repairable()
            && (info.repair_requirements() * vpr.part().damage_level(4))
                   .can_make_with_inventory(inv, is_crafting_component)) {
            repairable_cache[&vpr.part()] = repairable_status::repairable;
        }
    }

    const auto part_damage_comparison =
    ...
    const auto high_damage_iterator =
        std::max_element(vp_range.begin(), vp_range.end(), part_damage_comparison);
    if (high_damage_iterator == vp_range.end() || high_damage_iterator->part().removed
        || !high_damage_iterator->info().is_repairable()
        || (only_repairable
            && repairable_cache[&(*high_damage_iterator).part()]
                   != repairable_status::not_repairable)) {
...
bool repair_part(vehicle& veh, vehicle_part& pt, Character& who_c) {
    ...
    const std::string startdurability =
        colorize(pt.get_base().damage_symbol(), pt.get_base().damage_color());
    ...
    who.add_msg_if_player(
        ...
        wasbroken ? _("You replace the %1$s's %2$s. (was %3$s)")
                  : _("You repair the %1$s's %2$s. (was %3$s)"),

...
auto mps_to_cmps(double mps) -> int { return std::lround(mps * 100.0); }
...
int vehicle::slowdown(int at_velocity) const {
    ...
    if (is_watercraft()) {
        ...
        double f_rolling_drag =
            coeff_rolling_drag() * (vehicles::rolling_constant_to_variable + mps);
        if (vehicle_movement::is_on_rails(get_map(), *this)) {
            // vehicles on rails don't skid
            f_total_drag += f_rolling_drag;
        } else {
            // increase rolling resistance by up to 25x if the vehicle is skidding at right angle to
            // facing
    ...
    if (is_towing()) {
        ...
        if (other_veh) { slowdown += other_veh->slowdown(at_velocity); }
    }
    if (slowdown < 0) { debugmsg("vehicle %s has negative drag slowdown %d\n", name, slowdown); }
    add_msg(m_debug, "%s at %d cm/s, f_drag %3.2f, drag accel %.1f cm/s - extra drag %d", name,
            at_velocity, f_total_drag, slowdown, static_drag());
    // plows slow rolling vehicles, but not falling or floating vehicles
    if (!(is_falling || is_floating || is_flying)) { slowdown -= static_drag(); }
...
void vehicle::thrust(int thd, int z) {
    ...
    if ((in_water && can_float()) || (is_aircraft() && (z != 0 || is_flying))) {
        ...
        stop();
        if (pl_ctrl) { add_msg(_("The %s is too leaky!"), name); }
        return;
    } else if (!valid_wheel_config() && z == 0 && !has_sufficient_lift(true)) {
        stop();
        if (pl_ctrl) { add_msg(_("The %s doesn't have enough wheels to move!"), name); }
        return;
    }
    ...
    // Both balloons and wings can "glide", making themselves have times where they wont have engine
    // power As they can be without propellers
    ...
    // rotorcraft need to spend +5% (in addition to idle) of load to fly, +20% (in addition to idle)
    // to ascend
    ...
    if (thrusting && z != 0 && is_aircraft()) { requested_z_change = z; }
    ...
    if (load >= 1 && thrusting) {
        ...
        if (total_power_w() <= 0 || !engine_on || accel == 0) {
            ...
            for (size_t e = 0; e < engines.size(); e++) { do_engine_damage(e, strn); }
        }
    }
        ...
        if (vp.info().fuel_type == fuel_type_animal && engines.size() != 1) {
            ...
            if (mon != nullptr && mon->has_effect(effect_harnessed)) {
                if (velocity > mon->get_speed() * 12) {
                    add_msg(m_bad, _("Your %s is not fast enough to keep up with the %s"),
                            mon->get_name(), name);
                    int dmg = rng(0, 10);
                    damage_direct(e, dmg);
                }
...
void vehicle::turn(units::angle deg) {
    if (deg == 0_degrees) { return; }
    if (velocity < 0 && !::get_option<bool>("REVERSE_STEERING")) { deg = -deg; }
    last_turn = deg;
...
void vehicle::stop(bool update_cache) {
    ...
    last_turn = 0_degrees;
    of_turn_carry = 0;
    if (!update_cache) { return; }
    map& here = get_map();
    for (const auto& p : get_points()) { here.set_memory_seen_cache_dirty(abs_to_bub(p)); }
}
...
auto vehicle::collision(const vehicle_collision_options& options) -> bool {
    ...
    if (coll_velocity == 0 && !is_aircraft()) { just_detect = true; }
    ...
    for (int p = 0; static_cast<size_t>(p) < parts.size(); p++) {
        ...
        const auto dsp =
            bub_ms_location() + dp
            + tripoint_rel_ms(parts[p].precalc[1], parts[p].mount.z() + parts[p].z_terrain[1]);
        ...
        if (coll.type == veh_coll_nothing) { continue; }
        ...
        // A hack for falling vehicles: restore the velocity so that it hits at full force
        // everywhere
    ...
    if (vertical) {
        ...
        if (vertical_velocity == 0) { is_falling = false; }
    }
...
static void terrain_collision_data(
    const tripoint_bub_ms& p, bool bash_floor, float& mass, float& density, float& elastic) {
    elastic = 0.30;
    map& here = get_map();
...
auto vehicle::part_collision(const vehicle_part_collision_options& options) -> veh_collision {
    ...
    if (critter == options.ignored_critter) { critter = nullptr; }
    ...
    int& coll_velocity =
        (part_info(part).rotor_diameter() == 0)
            ? (vert_coll ? vertical_velocity : velocity)
            : rotor_velocity;
    if (!just_detect && coll_velocity == 0) { return ret; }

    if (is_body_collision) {
        // critters on a BOARDABLE part in this vehicle aren't colliding
        if (ovp && (&ovp->vehicle() == this) && get_pet(ovp->part_index())) { return ret; }
        ...
        if (here.has_flag("SWIMMABLE", critter->bub_pos())) {
            ...
            std::set<tripoint_abs_ms>& cur_points = get_points(true);
            // push the animal out of way until it's no longer in our vehicle and not in
            // anyone else's position
            while (g->critter_at(end_pos, true) || cur_points.contains(bub_to_abs(end_pos))) {
                start_pos = end_pos;
    ...
    if (armor_part >= 0) { ret.part = armor_part; }
    ...
    if (is_body_collision) {
        ...
        (bash_floor && here.is_bashable_ter_furn(p, true))
    ...
    } else if (here.impassable_ter_furn(p) || (bash_floor && !here.has_flag(TFLAG_NO_FLOOR, p))) {
        // not destructible
    ...
    const float mass =
        (part_info(part).rotor_diameter() > 0)
            ? to_kilogram(parts[part].base->weight())
    ...
    float vpart_dens = 0;
    if (!mats.empty()) {
        for (auto& mat_id : mats) { vpart_dens += mat_id.obj().density(); }
        // average
        vpart_dens /= mats.size();
    }

    // Calculates density factor. Used as a bad stand in to determine deformation distance.
    // Ranges from 0.1 -> 100, measured in cm. A density difference of 100 is needed for the full
    // value.
    float density_factor = std::abs(part_dens - vpart_dens);
    density_factor = clamp(density_factor, 0.1f, 100.0f);

    // Deformation distance of the collision, measured in meters. A bad approximation for a value we
    // dont have that would take intensive simulation to determine.
    ...
    do {
        ...
        vel1_a = (mass * vel1 + mass2 * vel2 + e * mass2 * (vel2 - vel1)) / (mass + mass2);
        ...
        // Due to conservation of momentum, both of these values should be equal. If not, something
        // odd has happened and physics will be wonky. Small threshold for rounding errors.
        if (std::abs(impulse_obj - impulse_veh) > 5) {
            ...
            if (std::fabs(vel2_a) > std::fabs(vel2)) { vel2 = vel2_a; }
        ...
        if (critter == nullptr || !critter->is_hallucination()) {
            ...
            if (bash_max != 0) { part_dmg = std::min(bash_max / dmg_mod, part_dmg); }
        ...
        if (ret.type == veh_coll_bashable) {
            ...
            smashed =
                here.is_bashable_ter_furn(p, false)
                && here.bash_resistance(p, bash_floor) <= obj_dmg
                && here.bash(p, obj_dmg, false, false, false, this).success;
            if (smashed) {
                // Experimental: only use as much energy as was required to destroy the obstacle,
                // and recalc impulse and momentum off of that. Recalculate vel1_a from new impulse.
                // Remember that the impulse from a collision is technically negative, reducing
                // speed.
                float old_veh_imp = impulse_veh;
                impulse_veh = damage_to_impulse(std::min(part_dmg, bash_max / dmg_mod));
                add_msg(m_debug, "Terrain collision impulse recovery of %.2f Ns",
                        old_veh_imp - impulse_veh);
            ...
            time_stunned = time_duration::from_turns(
                (rng(0, obj_dmg) > 10) + (rng(0, obj_dmg) > 40));
            if (time_stunned > 0_turns) { critter->add_effect(effect_stunned, time_stunned); }

            if (ph != nullptr) {
                ph->hitall(obj_dmg, 40, driver);
            } else {
                const int armor =
                    part_flag(ret.part, "SHARP")
                ...
                // Limit vehicle damage to the max health of the critter, attenuated by the damage
                // modifier.
                ...
                add_msg(m_debug, "Critter collision! %1s was hit by %2s", critter->disp_name(),
                        name);
                add_msg(m_debug,
                        "Vehicle of %.2f Kg at %2.f m/s impacted critter of %.2f Kg at %.2f m/s",
                        mass, vel1, mass2, vel2);
                add_msg(
                    m_debug,
        ...
        if (critter == nullptr || !critter->is_hallucination()) {
            ...
            coll_velocity =
                (!smashed && std::abs(new_velocity) >= std::abs(coll_velocity)) ? 0 : new_velocity;
        }
        ...
        if (!critter->is_hallucination()) {
            if (pl_ctrl) {
                if (time_stunned > 0_turns) {
                    ...
                    add_msg(m_warning, _("Your %1$s's %2$s rams into %3$s and stuns it!"), name,
                            parts[ret.part].name(), ret.target_name);
                } else {
                    //~ 1$s - vehicle name, 2$s - part name, 3$s - NPC or monster
                    add_msg(m_warning, _("Your %1$s's %2$s rams into %3$s!"), name,
                            parts[ret.part].name(), ret.target_name);
                }
            }
...
void vehicle::handle_trap(const tripoint_bub_ms& p, int part) {
    ...
    if (seen) {
        if (known) {
            ...
            add_msg(m_bad, _("The %1$s's %2$s runs over %3$s."), name, parts[part].name(),
                    tr.name());
        } else {
    ...
    for (const tripoint_abs_ms& abs : get_points(true)) {
        ...
        if (here.impassable_ter_furn(below) || here.has_flag_ter_or_furn(TFLAG_RAMP_DOWN, below)
            ...
            who.add_msg_if_player(
                m_bad, _("It would be unsafe to try and land when there are obstacles below you."));
            return false;
        }
    }
    return true;
...
bool vehicle::check_heli_ascend(Character& who) {
    ...
    if (!is_flying_in_air() && check_on_ramp()) {
        who.add_msg_if_player(
            m_bad, _("It would be unsafe to try and take off from an uneven surface."));
        return false;
    }
    map& here = get_map();
    for (const tripoint_abs_ms& abs : get_points(true)) {
        ...
        if (part_info(part_at(pt - bub_ms_location())).has_flag(VPFLAG_NOCOLLIDEABOVE)) {
            continue;
        }
        ...
        if (has_ceiling || has_blocking_ter_furn || has_veh || has_critter) {
            direction obstacle_direction = direction_from((pt - who.bub_pos()).xy().raw());
            ...
            if (has_ceiling) {
                ...
                blocker_string =
                    here.ter(above)->movecost == 0 ? here.tername(above) : here.furnname(above);
            } else if (has_veh) {
            ...
            if (obstacle_direction == direction::CENTER) {
                who.add_msg_if_player(
                    m_bad, _("Your ascent is blocked by %s directly above you."), blocker_string);
            } else {
                who.add_msg_if_player(
                    m_bad, _("Your ascent is blocked by %s to your %s."), blocker_string,
                    direction_string);
            }
            return false;
        }
    ...
    if (turn_delta != 0_degrees) {
        ...
        if (eff == -2) {
            driver.add_msg_if_player(
                m_info, _("You cannot steer an animal-drawn vehicle with no animal harnessed."));
            return;
        }

        if (eff < 0) {
            driver.add_msg_if_player(
                m_info, _("This vehicle has no steering system installed, you can't turn it."));
            return;
        }
        ...
        float skill = std::min(
            10.0f,
            driver.get_skill_level(skill_driving) + (driver.get_dex() + driver.get_per()) / 10.0f);
        ...
        if (penalty > skill || cost > 400) {
            ...
            if (cost >= driver.get_speed() * 2) {
                driver.add_msg_if_player(
                    m_warning, _("It takes you a very long time to steer that vehicle!"));
            } else if (cost >= driver.get_speed() * 1.5f) {
                driver.add_msg_if_player(
                    m_warning, _("It takes you a long time to steer that vehicle!"));
            }
        }
        ...
        driver.moves -= action_time_scale::
            vehicle_control_cost(driver, std::max(cost, driver.get_speed() / 3 + 1));
    }
...
void vehicle::possibly_recover_from_skid() {
    if (last_turn > 13_degrees) {
        // Turning on the initial skid is delayed, so move==face, initially. This filters out that
        // case.
        return;
    }
...
vehicle* vehicle::act_on_map() {
    ...
    if (decrement_summon_timer()) { return nullptr; }
    Character& player_character = get_player_character();
    const bool pl_ctrl = player_in_control(player_character);
    ...
    if (is_floating && !can_float()) {
        if (has_sufficient_lift()) {
            ...
            if (pl_ctrl) { unboard_all(); }
            if (g->remoteveh() == this) { g->setremoteveh(nullptr); }

            {
                ...
                tripoint_bub_sm sink_sm_min = tripoint_bub_sm::max();
                tripoint_bub_sm sink_sm_max = tripoint_bub_sm::min();
                std::ranges::for_each(
    ...
    // If the movement is due to a change in z-level, i.e a helicopter then the lateral movement
    // will often be zero.
    ...
    if (!falling_only) { of_turn -= turn_cost; }
    ...
    if (one_in(10)) {
        ...
        for (int boarded : boarded_parts()) {
            if (part_with_feature(boarded, VPFLAG_CONTROLS, true) >= 0) {
                ...
                if (passenger != nullptr) { passenger->practice(skill_driving, 1); }
            }
        }
    ...
    if (rpres.do_turn) { turn_dir = rpres.turn_dir; }
...
static auto ramp_z_delta_at(const map& here, const tripoint_bub_ms& pos) -> int {
    if (here.has_flag(TFLAG_RAMP_UP, pos)) { return 1; }
    if (here.has_flag(TFLAG_RAMP_DOWN, pos)) { return -1; }
    return 0;
}
...
void vehicle::check_falling_or_floating() {
    ...
    for (const auto& abs : pts) {
        ...
        if (is_falling) {
            is_falling &=
                here.has_flag_ter_or_furn(TFLAG_NO_FLOOR, p) && (p.z() > -OVERMAP_DEPTH)
    ...
    if (is_floating) { is_flying = false; }
...
static auto has_rail_at_vehicle_z(const map& m, const tripoint_bub_ms& p) -> bool {
    if (m.has_flag_ter_or_furn(TFLAG_RAIL, p)) { return true; }
    if (!m.has_flag_ter_or_furn(TFLAG_NO_FLOOR, p)) { return false; }
...
static bool scan_rails_at_shift(
    ...
    if (ray_delta.x != 0 && ray_delta.y != 0) {
        ...
        if (scan_res_l || scan_res_r) {
            if (shift_amt) { *shift_amt = scan_start - veh.bub_ms_location(); }
            return true;
        }
    } else {
        ...
        point scan_vec = ray_delta * velocity_sign;
        auto scan_start = veh.bub_ms_location();
        if (shift_sign != 0) { scan_start += scan_vec + veh_plus_y_vec * shift_sign; }
...
rail_processing_result process_movement_on_rails(const map& m, const vehicle& veh) {
    ...
    tripoint_rel_ms shift_amount_right;
    tripoint_rel_ms shift_amount_left;

    bool can_shift_right =
        scan_rails_at_shift(m, veh, vel_sign, dir_straight, vel_sign, &shift_amount_right);
    bool can_shift_left =
        scan_rails_at_shift(m, veh, vel_sign, dir_straight, -vel_sign, &shift_amount_left);

    bool is_on_rails =
        face_dir_degrees == face_dir_snapped && (can_go_straight || can_go_backwards);

    // Appraise possible vehicle orientations
    if (!is_on_rails) {
        // The vehicle is derailed, attempt to get back on rails
        if (can_go_straight) { return make_turn(dir_straight); }
    } else {

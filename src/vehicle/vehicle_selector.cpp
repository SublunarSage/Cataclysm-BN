...
vehicle_selector::vehicle_selector(
    const tripoint_bub_ms& pos, int radius, bool accessible, bool visibility_only) {
    map& here = get_map();
    for (const tripoint_bub_ms& e : closest_points_first(pos, radius)) {
        if (!accessible
            || (visibility_only ? here.sees(pos, e, radius)
                                : here.clear_path(pos, e, radius, 1, 100))) {
...
vehicle_selector::vehicle_selector(
    const tripoint_bub_ms& pos, int radius, bool accessible, const vehicle& ignore) {
    map& here = get_map();
    for (const tripoint_bub_ms& e : closest_points_first(pos, radius)) {

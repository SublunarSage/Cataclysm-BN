...
void vehicle::add_toggle_to_opts(
    ...
    auto found =
        get_avail_parts(flag) | filter(is_toggleable) | transform(&vpart_position::part_index)
    ...
    const auto state = ranges::none_of(found, [this](const auto p) { return parts[p].enabled; });

    // if toggled part potentially usable check if could be enabled now (sufficient fuel etc.)
    if (state) {
        allow = ranges::any_of(found, [this](const auto p) { return can_enable(parts[p]); });
    }

    auto msg = string_format(state ? _("Turn on %s") : colorize(_("Turn off %s"), c_pink), name);
...
void vehicle::control_doors() {
    ...
    pmenu.addentry(
        doors_with_motors.size() + OPENCURTAINS, true, MENU_AUTOASSIGN, _("Open all curtains"));
    pmenu.addentry(doors_with_motors.size() + OPENBOTH, true, MENU_AUTOASSIGN,
                   _("Open all curtains and doors"));
    pmenu.addentry(
        doors_with_motors.size() + CLOSEDOORS, true, MENU_AUTOASSIGN, _("Close all doors"));
        ...
        if (pmenu.ret < static_cast<int>(doors_with_motors.size())) {
            ...
            for (const vpart_reference& vp : door_motors) {
                ...
                if (open) {
                    ...
                    if (part != -1) {
                        if (!part_flag(part, "CURTAIN") && option == OPENCURTAINS) { continue; }
                        open_or_close(part, open);
                        if (option == OPENBOTH) { next_part = next_part_to_open(motor); }
                        if (next_part != -1) { open_or_close(next_part, open); }
                    }
                } else {
                    int part = next_part_to_close(motor);
                    if (part != -1) {
                        if (part_flag(part, "CURTAIN") && option == CLOSEDOORS) { continue; }
                        open_or_close(part, open);
                        if (option == CLOSEBOTH) { next_part = next_part_to_close(motor); }
                        if (next_part != -1) { open_or_close(next_part, open); }
                    }
                }
            }
...
void vehicle::set_electronics_menu_options(
    ...
    add_toggle(pgettext("electronics menu option", "reactor"), keybind("TOGGLE_REACTOR"), "REACTOR");
    add_toggle(pgettext("electronics menu option", "door locks"), keybind("TOGGLE_DOOR_LOCKS"),
               "DOOR_LOCKING");
    add_toggle(pgettext("electronics menu option", "headlights"), keybind("TOGGLE_HEADLIGHT"),
               "CONE_LIGHT");
    ...
    add_toggle(pgettext("electronics menu option", "aisle lights"), keybind("TOGGLE_AISLE_LIGHT"),
               "AISLE_LIGHT");
    add_toggle(pgettext("electronics menu option", "dome lights"), keybind("TOGGLE_DOME_LIGHT"),
               "DOME_LIGHT");
    add_toggle(pgettext("electronics menu option", "atomic lights"), keybind("TOGGLE_ATOMIC_LIGHT"),
               "ATOMIC_LIGHT");
    add_toggle(pgettext("electronics menu option", "stereo"), keybind("TOGGLE_STEREO"), "STEREO");
    add_toggle(pgettext("electronics menu option", "chimes"), keybind("TOGGLE_CHIMES"), "CHIMES");
    add_toggle(pgettext("electronics menu option", "fridge"), keybind("TOGGLE_FRIDGE"), "FRIDGE");
    add_toggle(pgettext("electronics menu option", "freezer"), keybind("TOGGLE_FREEZER"), "FREEZER");
    add_toggle(pgettext("electronics menu option", "space heater"), keybind("TOGGLE_SPACE_HEATER"),
               "SPACE_HEATER");
    add_toggle(pgettext("electronics menu option", "cooler"), keybind("TOGGLE_COOLER"), "COOLER");
    add_toggle(pgettext("electronics menu option", "recharger"), keybind("TOGGLE_RECHARGER"),
               "RECHARGE");
    add_toggle(pgettext("electronics menu option", "plow"), keybind("TOGGLE_PLOW"), "PLOW");
    add_toggle(pgettext("electronics menu option", "reaper"), keybind("TOGGLE_REAPER"), "REAPER");
    add_toggle(pgettext("electronics menu option", "planter"), keybind("TOGGLE_PLANTER"), "PLANTER");
    add_toggle(pgettext("electronics menu option", "rockwheel"), keybind("TOGGLE_PLOW"),
               "ROCKWHEEL");
    add_toggle(pgettext("electronics menu option", "roadheader"), keybind("TOGGLE_PLOW"),
               "ROADHEAD");
    add_toggle(pgettext("electronics menu option", "scoop"), keybind("TOGGLE_SCOOP"), "SCOOP");
    add_toggle(pgettext("electronics menu option", "converter"), keybind("TOGGLE_WATER_PURIFIER"),
               "CONVERTER");
...
void vehicle::control_electronics() {
    ...
    do {
        ...
        if (valid_option) { actions[menu.ret](); }
    } while (valid_option);
}
...
void vehicle::control_engines() {
    ...
    do {
        ...
        if (e_toggle < 0 || e_toggle >= fuel_count) { break; }
        dirty = true;
...
int vehicle::select_engine() {
    ...
    const auto get_title = [](const std::string& s) -> uilist_entry {
        auto title = uilist_entry(s).with_enabled(false).with_txt_color(c_cyan);
        title.force_color = true; // ui.h does not have force_color, adding it would recompile
                                  // everything
        return title;
    };
    ...
    for (size_t x = 0; x < engines.size(); x++) { entry_alt_fuels(x); }
...
void vehicle::smash_security_system() {
    ...
    if (c >= 0 && s >= 0) {
        ...
        if (percent_alarm > rand) {
            ...
            if (percent_alarm / 4 > rand || parts[s].is_broken()) { is_alarm_on = false; }
        }
...
void vehicle::toggle_autopilot() {
    uilist smenu;
    enum autopilot_option : int { PATROL, FOLLOW, STOP };
    smenu.desc_enabled = true;
    smenu.text = _("Choose action for the autopilot");
    smenu.addentry_col(
        PATROL, true, 'P', _("Patrol…"), "",
        string_format(_(
            "Program the autopilot to patrol a nearby vehicle patrol zone.  "
            "If no zones are nearby, you will be prompted to create one.")));
    smenu.addentry_col(
        FOLLOW, true, 'F', _("Follow…"), "",
        string_format(_(
            "Program the autopilot to follow you.  It might be a good idea to have a remote control available to tell it to stop, too.")));
    smenu.addentry_col(
        STOP, true, 'S', _("Stop…"), "",
    ...
    switch (smenu.ret) {
        ...
        case FOLLOW: {
            ...
            const auto initial_follow_distance =
                follow_distance > 0 ? follow_distance : default_follow_distance;
            const auto requested_follow_distance =
                string_input_popup()
                    ...
                    .query_int();
            follow_distance =
                requested_follow_distance > 0 ? requested_follow_distance : default_follow_distance;
            start_engines();
            refresh();
            break;
        }
...
void vehicle::use_controls(const tripoint_bub_ms& pos) {
    ...
    if (remote) {
        ...
        has_electronic_controls =
            !get_parts_at(pos, "CTRL_ELECTRONIC", part_status_flag::any).empty();
    }
    ...
    if (has_part("ENGINE")
        && !(is_flying && units::to_newton(total_mass()) >= total_balloon_lift()
             && has_part(VPFLAG_ROTOR))) {
    ...
    actions.emplace_back([&] { set_cruise_control_speed(); });
    ...
    if (has_electronic_controls) {
        set_electronics_menu_options(options, actions);
        options
            .emplace_back(_("Control multiple electronics"), keybind("CONTROL_MANY_ELECTRONICS"));
        actions.emplace_back([&] {
            control_electronics();
            refresh();
        });
    ...
    if (has_part("TURRET")) {
        ...
        if (!turrets.empty()) {
            ...
            actions.emplace_back([&] {
                turrets_aim_and_fire_mult(you, turret_filter_types::MANUAL, true);
                refresh();
            });
            ...
            actions.emplace_back([&] {
                turrets_aim_and_fire_mult(you, turret_filter_types::AUTOMATIC, true);
                refresh();
            });
            ...
            actions.emplace_back([&] {
                turrets_aim_and_fire_mult(you, turret_filter_types::BOTH, true);
                refresh();
            });

            options.emplace_back(_("Aim individual turret"), keybind("TURRET_SINGLE_FIRE"));
            actions.emplace_back([&] {
                turrets_aim_and_fire_single(you);
                refresh();
            });
        }
    }
    ...
    if (!droppers.empty()) {
        options.emplace_back(
            _("Activate all item droppers (Drop Everything)"), keybind("DROPPER_ALL"));
        actions.emplace_back([&] {
            item_dropper_drop_all();
            refresh();
        });

        options.emplace_back(
            _("Activate one item dropper (Drop Everything)"), keybind("DROPPER_SINGLE_ALL"));
        actions.emplace_back([&] {
            item_dropper_drop_single(false);
            refresh();
        });

        options.emplace_back(
            _("Activate one item dropper (Drop One Thing)"), keybind("DROPPER_SINGLE"));
        actions.emplace_back([&] {
            item_dropper_drop_single(true);
            refresh();
        });
    }
    uilist menu;
...
bool vehicle::fold_up() {
    ...
    // Create the item
    detached_ptr<item> folding_veh_item =
        item::spawn(can_be_folded ? "generic_folded_vehicle" : itype_id, calendar::turn);
    ...
    } catch (const JsonError& e) { debugmsg("Error storing vehicle: %s", e.c_str()); }
    ...
    if (can_be_folded) {
        ...
        // remove "folded" from name to allow for more flexibility with folded vehicle names. also
...
double vehicle::engine_cold_factor(const int e) const {
    ...
    if (!parts[engines[e]].faults().contains(fault_glowplug)) { eff_temp = std::min(eff_temp, 20); }
...
bool vehicle::start_engine(const int e) {
    ...
    const bool out_of_fuel = [&] {
        if (einfo.fuel_type == fuel_type_none || engine_fuel_left(e) > 0) { return false; }
    ...
    if (eng.faults().contains(fault_immobiliser)) {
        ...
        se.id = "vehicle";
        se.variant = "fault_immobiliser_beep";
        sounds::sound(se);
        return false;
    }
    ...
    if (eng.faults_potential().contains(fault_starter)) {
        if (eng.faults().contains(fault_starter)) {
            ...
            se.id = "vehicle";
            se.variant = "engine_single_click_fail";
            sounds::sound(se);
            return false;
        }
        // TODO: start_moves is in moves, but it's an integer, convert it to some time class
        const int start_draw_bat = power_to_energy_bat(
            engine_power * (1.0 + dmg / 2 + cold_factor / 5) * 10, 1_turns * start_moves / 100);
        if (discharge_battery(start_draw_bat, true) != 0) {
            ...
            se.category = sounds::sound_t::alarm;
            se.movement_noise = true;
            se.description = string_format(_("the %s rapidly clicking"), eng.name());
            se.id = "vehicle";
            se.variant = "engine_multi_click_fail";
            sounds::sound(se);
            return false;
        }
    }
    ...
    if (eng.faults().contains(fault_pump) || eng.faults().contains(fault_diesel)) {
        ...
        se.category = sounds::sound_t::movement;
        se.movement_noise = true;
        se.description = string_format(_("the %s quickly stuttering out."), eng.name());
        se.id = "vehicle";
        se.variant = "engine_stutter_fail";
        sounds::sound(se);
        return false;
    }
    ...
    if (!(is_engine_type(e, fuel_type_battery) || is_engine_type(e, fuel_type_muscle))
        ...
        se.category = sounds::sound_t::movement;
        se.movement_noise = true;
        se.description = string_format(_("the %s clanking and grinding"), eng.name());
        se.id = "vehicle";
        se.variant = "engine_clanking_fail";
        sounds::sound(se);
        return false;
    }
    sound_event se;
    ...
    se.category = sounds::sound_t::movement;
    se.movement_noise = true;
    se.description = string_format(_("the %s starting"), eng.name());
    sounds::sound(se);
...
void vehicle::start_engines(const bool take_control, const bool autodrive) {
    ...
    if (!has_engine) {
        for (auto idx : engines) {
            if (!parts[idx].is_broken()) { parts[idx].enabled = true; }
        }
    }

    int start_time = 0;
    // record the first usable engine as the referenced position checked at the end of the engine
    // starting activity
...
void vehicle::honk_horn() {
    ...
    for (const vpart_reference& vp : get_avail_parts("HORN")) {
        ...
        if ((horn_type.get_id() != vpart_id("horn_bicycle")) && no_power) { continue; }
        if (!honked) {
        ...
        if (horn_type.bonus >= 110) {
            //~ Loud horn sound
            se.description = _("HOOOOORNK!");
            se.variant = "horn_loud";
            sounds::sound(se);
        } else if (horn_type.bonus >= 80) {
            //~ Moderate horn sound
            se.description = _("BEEEP!");
            se.variant = "horn_medium";
            sounds::sound(se);
        } else {
            //~ Weak horn sound
            se.description = _("honk.");
            se.variant = "horn_low";
            sounds::sound(se);
        }
    }

    if (!honked) { add_msg(_("You honk the horn, but nothing happens.")); }
}

void vehicle::reload_seeds(const tripoint_bub_ms& pos) {
    player& p = g->u;

    std::vector<item*> seed_inv = p.items_with([](const item& itm) { return itm.is_seed(); });
    ...
    if (seed_index > 0 && seed_index < static_cast<int>(seed_entries.size())) {
        ...
        int amount = 0;
        const std::string popupmsg =
            string_format(_("Move how many?  [Have %d] (0 to cancel)"), count);

        amount = string_input_popup().title(popupmsg).width(5).only_digits(true).query_int();
...
void vehicle::beeper_sound() {
    // No power = no sound
    if (fuel_left(fuel_type_battery, true) == 0) { return; }
...
void vehicle::crash_terrain_around() {
    ...
    for (const vpart_reference& vp : get_enabled_parts("CRASH_TERRAIN_AROUND")) {
        ...
        for (size_t i = 0;
             i < eight_horizontal_neighbors.size() && !g->m.inbounds_z(crush_target.z()); i++) {
...
void vehicle::transform_terrain() {
    for (const vpart_reference& vp : get_enabled_parts("TRANSFORM_TERRAIN")) {
        ...
        if (prereq_fulfilled) {
            ...
            if (new_ter != t_null) { g->m.ter_set(start_pos, new_ter); }
            const furn_id new_furn = furn_id(ttd.post_furniture);
            if (new_furn != f_null) { g->m.furn_set(start_pos, new_furn); }
...
void vehicle::operate_reaper() {
    for (const vpart_reference& vp : get_enabled_parts("REAPER")) {
        ...
        if (g->m.furn(reaper_pos) != f_plant_harvest) { continue; }
        // Can't use item_stack::only_item() since there might be fertilizer
        map_stack items = g->m.i_at(reaper_pos);
        map_stack::iterator seed =
            std::find_if(items.begin(), items.end(), [](const item* const& it) {
                return it->is_seed();
            });
        ...
        for (auto& i :
             iexamine::get_harvest_items(seed_type, plant_produced, seed_produced, false)) {
        ...
        if (vp.has_feature("CARGO")) {
            items.remove_top_items_with(
                [&max_pickup_volume, this, reaper_id](detached_ptr<item>&& it) {
...
void vehicle::operate_planter() {
    for (const vpart_reference& vp : get_enabled_parts("PLANTER")) {
        ...
        for (auto it = v.begin(); it != v.end(); it++) {
            ...
            item* i = *it;
            if (i->is_seed()) {
                // If it is an "advanced model" then it will avoid damaging itself or becoming
                // damaged. It's a real feature.
...
void vehicle::operate_scoop() {
    for (const vpart_reference& vp : get_enabled_parts("SCOOP")) {
        ...
        for (const tripoint_bub_ms& current : g->m.points_in_radius(bub_part_location(scoop), 1)) {
            parts_points.push_back(current);
        }
        for (const tripoint_bub_ms& position : parts_points) {
            g->m.mop_spills(position);
            if (!g->m.has_items(position)) { continue; }
            ...
            if (!that_item_there) { continue; }
            if (one_in(chance_to_damage_item)
                && that_item_there->damage() < that_item_there->max_damage()) {
                ...
                se.origin = position;
                se.volume = std::min(
                    90, rng(30, that_item_there->volume() / units::legacy_volume_factor * 2 + 30));
                se.category = sounds::sound_t::combat;
                se.description = _("BEEEThump");
                se.id = "vehicle";
                se.variant = "scoop_thump";
                sounds::sound(se);
            }
            // This attempts to add the item to the scoop inventory and if successful, removes it
            // from the map.
            ...
            if (mon) {
                if (mon->is_player()) {
                    ...
                    who.add_msg_if_player(
                        m_info, _("%s is in the way!"), mon->disp_name(false, true));
                } else {
                    who.add_msg_if_player(
                        m_info, _("%s is in the way!"), mon->disp_name(false, true));
                }
...
void vehicle::open_or_close(const int part_index, const bool opening) {
    ...
    if (dist < 20) {
        sfx::play_variant_sound(
            opening ? "vehicle_open" : "vehicle_close", parts[part_index].info().get_id().str(),
            100 - dist * 3);
    }
    for (auto const& vec : find_lines_of_parts(part_index, "OPENABLE")) {
        for (auto const& partID : vec) { parts[partID].open = opening; }
    }
...
void vehicle::use_monster_capture(int part, const tripoint_bub_ms& pos) {
    if (parts[part].is_broken() || parts[part].removed) { return; }
...
void vehicle::use_harness(int part, const tripoint_bub_ms& pos) {
    if (parts[part].is_unavailable() || parts[part].removed) { return; }
    if (!g->is_empty(pos)) {
    ...
    const std::function<bool(const tripoint_bub_ms&)> f = [](const tripoint_bub_ms& pnt) {
        ...
        if (mon_ptr == nullptr) { return false; }
        monster& f = *mon_ptr;
        return (
            f.friendly != 0 && (f.has_flag(MF_PET_MOUNTABLE) || f.has_flag(MF_PET_HARNESSABLE)));
    };
...
void vehicle::interact_with(const tripoint_bub_ms& pos, int interact_part) {
    ...
    const auto turret_menu_name = [&turret]() -> std::string {
        ...
        if (ammo_data == nullptr) { return turret.base().tname(); }
    ...
    const bool has_electronics =
        avail_part_with_feature(interact_part, "CTRL_ELECTRONIC", true) >= 0;
    ...
    const int monster_capture_part =
        avail_part_with_feature(interact_part, "CAPTURE_MONSTER_VEH", true);
    ...
    if (has_electronics) {
        selectmenu.addentry(
            CONTROL_ELECTRONICS, true, keybind("CONTROL_MANY_ELECTRONICS"),
            _("Control multiple electronics"));
    }
    if (has_autoclave) { selectmenu.addentry(USE_AUTOCLAVE, true, 'a', _("Sterilize a CBM")); }
    if (has_autodoc) { selectmenu.addentry(USE_AUTODOC, true, 'I', _("Use autodoc")); }
    if (from_vehicle) { selectmenu.addentry(GET_ITEMS, true, 'g', _("Get items")); }
    ...
    if (has_harness) { selectmenu.addentry(USE_HARNESS, true, 'H', _("Harness an animal")); }
    if (has_planter) {
    ...
    auto veh_tool = [&](const itype_id& obj) {
        ...
        if (fuel_left(itype_battery, true) < pseudo.ammo_required()) { return false; }
    ...
    switch (choice) {
        ...
        case USE_CRAFTER: {
            ...
            if (tool_index < 0) { return; }
            item& fake_item =
                *item::spawn_temporary(usable_item_types.at(tool_index), calendar::turn, 0);

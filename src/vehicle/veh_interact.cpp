...
static double jack_quality(const vehicle& veh) {
    const units::quantity<double, units::mass::unit_type> mass =
        std::min(veh.total_mass(), JACK_LIMIT);
...
std::unique_ptr<player_activity> veh_interact::serialize_activity() {
    ...
    if (sel_cmd == 'q' || sel_cmd == ' ' || !vp) { return std::make_unique<player_activity>(); }
    ...
    if (you.has_trait(trait_DEBUG_HT)) { time = 1; }
...
std::unique_ptr<player_activity> veh_interact::run(vehicle& veh, tripoint_mnt_veh p) {
    veh_interact vehint(veh, p);
    vehint.do_main_loop();
    return vehint.serialize_activity();
}

vehicle_part& veh_interact::select_part(
    const vehicle& veh, const part_selector& sel, const std::string& title) {
    static vehicle_part null_part;
    vehicle_part* res = &null_part;

    auto act = [&](const vehicle_part& pt) { res = const_cast<vehicle_part*>(&pt); };
 ...
 */
veh_interact::veh_interact(vehicle& veh, tripoint_mnt_veh p)
    : vehicle_cursor(p),
      veh(&veh),
    ...
    move_cursor(tripoint_rel_veh::zero());
}

veh_interact::~veh_interact() { get_avatar().view_offset = stored_view_offset; }
...
bool veh_interact::format_reqs(
    ...
    if (skills.empty()) {
        //~ %1$s represents the internal color name which shouldn't be translated, %2$s is the word
        //"NONE"
...
shared_ptr_fast<ui_adaptor> veh_interact::create_or_get_ui_adaptor() {
    ...
    if (!current_ui) {
        ...
        current_ui->on_redraw([this](const ui_adaptor&) {
            ...
            werase(w_parts);
            veh->print_part_list(
                w_parts, 0, getmaxy(w_parts) - 1, getmaxx(w_parts), cpart, highlight_part, true,
                parts_list_offset);
            wnoutrefresh(w_parts);

            werase(w_msg);
            if (!msg.has_value()) {
                veh->print_vparts_descs(
                    w_msg, getmaxy(w_msg), getmaxx(w_msg), cpart, start_at, start_limit);
            } else {
...
void veh_interact::do_main_loop() {
    ...
    while (!finish) {
        ...
        if (const std::optional<tripoint_rel_ms> vec = main_context.get_direction(action)) {
            ...
            if (veh->handle_potential_theft(you)) { do_install(); }
        } else if (action == "REPAIR") {
            if (veh->handle_potential_theft(you)) { do_repair(); }
        } else if (action == "MEND") {
            if (veh->handle_potential_theft(you)) { do_mend(); }
        } else if (action == "REFILL") {
            if (veh->handle_potential_theft(you)) { do_refill(); }
        } else if (action == "REMOVE") {
            if (veh->handle_potential_theft(you)) { do_remove(); }
            ...
            if (veh->handle_potential_theft(you)) { finish = do_unload(); }
            ...
            move_cursor(tripoint_rel_veh::zero(), 1);
            ...
            if (parts_list_offset > 0) { parts_list_offset--; }
#if defined(TILES)
        } else if (action == "ZOOM_IN") {
            if (tile_preview) { tile_preview->zoom_in(); }
        } else if (action == "ZOOM_OUT") {
            if (tile_preview) { tile_preview->zoom_out(); }
#endif
        }
        if (sel_cmd != ' ') { finish = true; }
    }
}
...
void veh_interact::cache_tool_availability() {
    ...
    int mech_jack = 0;
    if (you.is_mounted()) { mech_jack = you.mounted_creature->mech_str_addition() + 10; }
    max_jack = std::max(
        {you.max_quality(qual_JACK), mech_jack,
...
task_reason veh_interact::cant_do(char mode) {
    ...
    if (cpart != -1 || cpart > veh->part_count()) {
        ...
        if (pt) {
            const auto q = veh->mount_to_abs(pt->mount);
            const vehicle* cacheveh = &g->m.veh_at(q)->vehicle();
            if (veh != cacheveh) { return DOUBLE_STACK; }
        }
    }
    switch (mode) {
        case 'i':
        ...
        case 'm': {
            ...
            valid_target =
                std::any_of(vpr.begin(), vpr.end(), [toggling](const vpart_reference& pt) {
                    if (toggling) {
        ...
        case 'w':
            // assign crew
            if (g->allies().empty()) { return INVALID_TARGET; }
            return std::any_of(vpr.begin(), vpr.end(),
                               [](const vpart_reference& e) { return e.part().is_seat(); })
                     ? CAN_DO
                     : INVALID_TARGET;
        ...
        case 'p': {
            ...
            if (cpart >= 0) {
                ...
                valid_target =
                    vpart_shapes[displayed_vpart.name() + displayed_vpart.item.str()].size() > 1;
            }
            break;
        }
        case 'a':
    ...
    if (std::abs(veh->velocity) > 100 || you.controlling_vehicle) { return MOVING_VEHICLE; }
    if (!valid_target) { return INVALID_TARGET; }
    if (!enough_morale) { return LOW_MORALE; }
    if (!enough_light) { return LOW_LIGHT; }
    if (!has_tools) { return LACK_TOOLS; }
    if (!part_free) { return NOT_FREE; }
    // TODO: that is always false!
    if (!has_skill) { return LACK_SKILL; }
    return CAN_DO;
}
...
bool veh_interact::is_drive_conflict() {
    ...
    if (has_conflict) {
        //~ %1$s is fuel_type
        msg = string_format(_("Only one %1$s powered engine can be installed."), conflict_type);
    }
    return has_conflict;
}
...
bool veh_interact::can_self_jack() {
    ...
    for (const vpart_reference& vp : veh->get_avail_parts("SELF_JACK")) {
        if (vp.part().base->has_quality(qual_SELF_JACK, lvl)) { return true; }
    }
    return false;
}

bool veh_interact::update_part_requirements() {
    if (sel_vpart_info == nullptr) { return false; }

    if (is_drive_conflict()) { return false; }
    ...
    const auto get_rq_mechanics = [](const vpart_info& vpi) {
        for (const std::pair<const skill_id, int>& it : vpi.install_skills) {
            if (it.first == skill_mechanics) { return it.second; }
        }
        return 0;
    };
    ...
    if (sel_vpart_info->has_flag("STEERABLE")) {
        ...
        if (!axles.empty() && !axles.contains(vehicle_cursor.x())) {
    ...
    if (dif_eng > 0) {
        if (you.get_skill_level(skill_mechanics) < dif_eng) { ok = false; }
    ...
    if (lifting_or_jacking_required) {
        ...
        additional_requirements +=
            string_format(_("> %1$s <color_white>OR</color> %2$s"), colorize(aid_string, aid_color),
...
void veh_interact::do_install() {
    ...
    restore_on_out_of_scope<std::unique_ptr<install_info_t>> prev_install_info(
        std::move(install_info));
    install_info = std::make_unique<install_info_t>();

    std::array<std::string, 8>& tab_list = install_info->tab_list = {
        {pgettext("Vehicle Parts|", "All"), pgettext("Vehicle Parts|", "Cargo"),
         pgettext("Vehicle Parts|", "Light"), pgettext("Vehicle Parts|", "Util"),
         pgettext("Vehicle Parts|", "Hull"), pgettext("Vehicle Parts|", "Internal"),
         pgettext("Vehicle Parts|", "Other"), pgettext("Vehicle Parts|", "Filter")}};

    install_info->tab_list_short = {
        {pgettext("Vehicle Parts|", "A"), pgettext("Vehicle Parts|", "C"),
         pgettext("Vehicle Parts|", "L"), pgettext("Vehicle Parts|", "U"),
         pgettext("Vehicle Parts|", "H"), pgettext("Vehicle Parts|", "I"),
         pgettext("Vehicle Parts|", "O"), pgettext("Vehicle Parts|", "F")}};
    ...
    tab_filters[2] = [&](const vpart_info* p) {
        ...
        return part.has_flag(VPFLAG_LIGHT) || // Light
               part.has_flag(VPFLAG_CONE_LIGHT) || part.has_flag(VPFLAG_WIDE_CONE_LIGHT)
            || part.has_flag(VPFLAG_CIRCLE_LIGHT) || part.has_flag(VPFLAG_DOME_LIGHT)
            || part.has_flag(VPFLAG_AISLE_LIGHT) || part.has_flag(VPFLAG_ATOMIC_LIGHT);
    };
    tab_filters[3] = [&](const vpart_info* p) {
        auto& part = *p;
        return part.has_flag("TRACK") || // Util
               part.has_flag(VPFLAG_FRIDGE) || part.has_flag(VPFLAG_FREEZER)
            || part.has_flag("CRAFTER") || part.has_flag("HOTPLATE") || part.has_flag("WELDRIG")
            || part.has_flag("HORN") || part.has_flag("BEEPER") || part.has_flag("AUTOPILOT")
            || part.has_flag("WATCH") || part.has_flag("ALARMCLOCK")
            || part.has_flag(VPFLAG_RECHARGE) || part.has_flag("VISION")
            || part.has_flag("POWER_TRANSFER") || part.has_flag("FAUCET") || part.has_flag("STEREO")
            || part.has_flag("CHIMES") || part.has_flag("MUFFLER")
            || part.has_flag("REMOTE_CONTROLS") || part.has_flag("CURTAIN")
            || part.has_flag("SEATBELT") || part.has_flag("SECURITY") || part.has_flag("SEAT")
            || part.has_flag("BED") || part.has_flag("SPACE_HEATER") || part.has_flag("COOLER")
            || part.has_flag("DOOR_MOTOR") || part.has_flag("WATER_PURIFIER")
            || part.has_flag("WORKBENCH");
    };
    tab_filters[4] = [&](const vpart_info* p) {
        auto& part = *p;
        return (part.has_flag(VPFLAG_OBSTACLE) || // Hull
                part.has_flag("ROOF") || part.has_flag(VPFLAG_ARMOR))
            && !part.has_flag("WHEEL") && !tab_filters[3](p);
    };
    tab_filters[5] = [&](const vpart_info* p) {
        auto& part = *p;
        return part.has_flag(VPFLAG_ENGINE) || // Internals
               part.has_flag(VPFLAG_ALTERNATOR) || part.has_flag(VPFLAG_CONTROLS)
            || part.location == "fuel_source" || part.location == "on_battery_mount"
        ...
        if (action == "INSTALL" || action == "CONFIRM") {
            if (can_install) {
                switch (reason) {
                    case DOUBLE_STACK:
                        msg = _(
                            "It's too hard to reach the vehicle with the other vehicle in the way");
                        return;
                    case LOW_MORALE:
                ...
                if (shapes.size() > 1) { // more than one shape available, display selection
                    ...
                    smenu.w_width_setup = [this]() { return getmaxx(w_list); };
                    smenu.w_x_setup = [this](const int) { return getbegx(w_list); };
                    smenu.w_y_setup = [this](const int) { return getbegy(w_list); };
                    smenu.query();
...
bool veh_interact::move_in_list(
    int& pos, const std::string& action, const int size, const int header) const {
...
void veh_interact::do_repair() {
    ...
    if (reason == INVALID_TARGET) {
        ...
        if (most_repairable && most_repairable->damage_percent()) {
            move_cursor((most_repairable->mount - vehicle_cursor));
            return;
        }
    }
...
void veh_interact::do_refill() {
    switch (cant_do('f')) {
        case DOUBLE_STACK:
            msg = _(
                "You can't refill that fuel tank, it's too hard to reach with the other vehicle in the way");
            return;
...
void veh_interact::calc_overview() {
    ...
    overview_headers["ENGINE"] = [this](const catacurses::window& w, int y) {
        trim_and_print(
            w, point(1, y), getmaxx(w) - 2, c_light_gray,
            string_format(
                _("Engines: %sSafe %4d kW</color> %sMax %4d kW</color>"), health_color(true),
                veh->total_power_w(true, true) / 1000, health_color(false),
                veh->total_power_w() / 1000));
    ...
    overview_headers["BATTERY"] = [epower_w](const catacurses::window& w, int y) {
        ...
        if (std::abs(epower_w) < 10000) {
            batt = string_format(
                _("Batteries: %s%+4d W</color>"), health_color(epower_w >= 0), epower_w);
        } else {
            batt = string_format(
                _("Batteries: %s%+4.1f kW</color>"), health_color(epower_w >= 0),
                epower_w / 1000.0);
        }
    ...
    overview_headers["POWER_GENERATOR"] = [this](const catacurses::window& w, int y) {
        ...
        for (const auto& vpr : veh->get_all_parts()) {
            if (vpr.part().is_available() && vpr.part().is_perpetual_power_source()
                && !vpr.part().is_reactor()) {
                generator_epower_w += static_cast<int>(
                    vpr.part().info().epower * vpr.part().health_percent());
            }
        }
        ...
        if (generator_epower_w == 0) {
            ...
            generator = string_format(
                _("Power generators: %s%+4d W</color>"), health_color(generator_epower_w),
                generator_epower_w);
        } else {
            generator = string_format(
                _("Power generators: %s%+4.1f kW</color>"), health_color(generator_epower_w),
                generator_epower_w / 1000.0);
        }
    ...
    overview_headers["REACTOR"] = [this, epower_w](const catacurses::window& w, int y) {
        ...
        for (const auto& vpr : veh->get_all_parts()) {
            if (vpr.part().is_available() && vpr.part().is_reactor()
                ...
                reactor_epower_w += static_cast<int>(
                    vpr.part().info().epower * vpr.part().health_percent());
            }
        }
        if (reactor_epower_w > 0 && epower_w < 0) { reactor_epower_w += epower_w; }
        std::string reactor;
        if (reactor_epower_w == 0) {
            reactor = _("Reactors");
        } else if (reactor_epower_w < 10000) {
            reactor = string_format(
                _("Reactors: Up to %s%+4d W</color>"), health_color(reactor_epower_w),
                reactor_epower_w);
        } else {
            reactor = string_format(
                _("Reactors: Up to %s%+4.1f kW</color>"), health_color(reactor_epower_w),
                reactor_epower_w / 1000.0);
        }
    ...
    for (const vpart_reference& vpr : veh->get_all_parts()) {
        if (vpr.part().is_engine() && vpr.part().is_available()) {
                ...
                right_print(
                    ...
                    string_format(
                        ...
                        right_justify(pt.enabled ? pgettext("vehicle part enabled value", "Yes")
                                                 : pgettext("vehicle part enabled value", "No"),
                                      3)));
            };
            ...
            overview_opts.emplace_back(
                "ENGINE", &vpr.part(), next_hotkey(vpr.part(), hotkey), details, msg_cb);
        }
    }
        ...
        if (vpr.part().is_tank() && vpr.part().is_available()) {
            auto details = [](const vehicle_part& pt, const catacurses::window& w, int y) {
                if (!pt.ammo_current().is_null()) {
                    ...
                    if (it.rotten()) { specials += _(" (rotten)"); }
            ...
            overview_opts
                .emplace_back("TANK", &vpr.part(), next_hotkey(vpr.part(), hotkey), details);
        } else if (
            vpr.part().is_fuel_store() && !(vpr.part().is_battery() || vpr.part().is_reactor())
            && !vpr.part().is_broken()) {
            auto details = [](const vehicle_part& pt, const catacurses::window& w, int y) {
                if (!pt.ammo_current().is_null()) {
                    const itype* pt_ammo_cur = &*pt.ammo_current();
    ...
    for (const vpart_reference& vpr : veh->get_all_parts()) {
        if (vpr.part().is_battery() && vpr.part().is_available()) {
            ...
            overview_opts
                .emplace_back("BATTERY", &vpr.part(), next_hotkey(vpr.part(), hotkey), details);
        }
    }
    ...
    auto details_power_generator = [](const vehicle_part& pt, const catacurses::window& w, int y) {
        right_print(
            w, y, 1, c_light_gray,
            string_format(
                _("%+d W     %s"), static_cast<int>(pt.info().epower * pt.health_percent()),
    ...
    for (const vpart_reference& vpr : veh->get_all_parts()) {
        if (vpr.part().is_perpetual_power_source() && !vpr.part().is_reactor()
            ...
            overview_opts.emplace_back(
                "POWER_GENERATOR", &vpr.part(), next_hotkey(vpr.part(), hotkey),
    ...
    auto details_ammo = [](const vehicle_part& pt, const catacurses::window& w, int y) {
        if (pt.ammo_remaining()) {
            ...
            right_print(
                w, y, offset, pt.ammo_current()->color,
    ...
    for (const vpart_reference& vpr : veh->get_all_parts()) {
        if (vpr.part().is_reactor() && vpr.part().is_available()) {
            overview_opts.emplace_back(
                "REACTOR", &vpr.part(), next_hotkey(vpr.part(), hotkey), details_ammo);
        }
    }
    ...
    for (const vpart_reference& vpr : veh->get_all_parts()) {
        if (vpr.part().is_turret() && vpr.part().is_available()) {
            overview_opts
                .emplace_back("TURRET", &vpr.part(), next_hotkey(vpr.part(), hotkey), details_ammo);
        }
    }
    ...
    for (const vpart_reference& vpr : veh->get_all_parts()) {
        ...
        if (vpr.part().is_seat() && vpr.part().is_available()) {
            overview_opts
                .emplace_back("SEAT", &vpr.part(), next_hotkey(vpr.part(), hotkey), details);
        }
    }
}
...
void veh_interact::display_overview() {
    ...
    int y = 0;
    if (overview_offset) {
        trim_and_print(w_list, point(1, y), getmaxx(w_list) - 1, c_yellow, _("'{' to scroll up"));
        y++;
    }
    for (int idx = overview_offset; idx != static_cast<int>(overview_opts.size()); ++idx) {
        ...
        trim_and_print(
            w_list, point(1, y), getmaxx(w_list) - 1, highlighted ? hilite(col) : col,
        ...
        if (y < (getmaxy(w_list) - 1)) {
            ...
            overview_limit = idx;
            trim_and_print(
                w_list, point(1, y), getmaxx(w_list) - 1, c_yellow, _("'}' to scroll down"));
            break;
        }
    }
...
void veh_interact::overview(const overview_enable_t& enable, const overview_action_t& action) {
    ...
    while (true) {
        ...
        const bool has_any_hotkey =
            std::any_of(overview_opts.begin(), overview_opts.end(), [](const part_option& e) {
                return e.hotkey;
            });
        if (!has_any_hotkey) {
        ...
        if (overview_pos >= 0 && static_cast<size_t>(overview_pos) < overview_opts.size()) {
            move_cursor((overview_opts[overview_pos].part->mount - vehicle_cursor));
        }
        ...
        if (input == "CONFIRM" && overview_opts[overview_pos].hotkey && overview_action) {
            ...
            do {
                move_overview_line(-1);
                if (--overview_pos < 0) { overview_pos = overview_opts.size() - 1; }
            } while (!overview_opts[overview_pos].hotkey);
        } else if (input == "DOWN") {
            do {
                move_overview_line(1);
                if (++overview_pos >= static_cast<int>(overview_opts.size())) { overview_pos = 0; }
            } while (!overview_opts[overview_pos].hotkey);
        } else {
            ...
            if (hotkey && overview_action) {
                auto iter = std::find_if(
                    overview_opts.begin(), overview_opts.end(),
...
vehicle_part* veh_interact::get_most_damaged_part() const {
    ...
    const vehicle_part_range vpr = veh->get_all_parts();
    auto high_damage_iterator = std::max_element(vpr.begin(), vpr.end(), part_damage_comparison);
    if (high_damage_iterator == vpr.end() || high_damage_iterator->part().removed) {
        return nullptr;
    }
...
bool veh_interact::can_remove_part(int idx, const Character& who) {
    ...
    if (who.in_vehicle) {
        if (const optional_vpart_position vp = g->m.veh_at(who.bub_pos())) {
            if (vp->is_inside()) { inside_vehicle = true; }
        }
    }

    if (!inside_vehicle && sel_vpart_info->has_flag("NOREMOVE_OUTSIDE")) {
        additional_requirements +=
            string_format(_("> %1$s%2$s</color>"), status_color(false),
...
void veh_interact::do_remove() {
    ...
    while (true) {
        ...
        overview_enable = [this, part](const vehicle_part& pt) { return &pt == &veh->part(part); };
...
void veh_interact::do_siphon() {
    ...
    auto act = [&](const vehicle_part& pt) {
        on_out_of_scope restore_ui([&]() { hide_ui(false); });
        hide_ui(true);
...
void veh_interact::do_change_shape() {
    if (cant_do('p') == task_reason::INVALID_TARGET) {
        msg = _(
            "No parts here or the part visible from your position does not have different shapes");
        return;
    }
...
void veh_interact::do_assign_crew() {
    ...
    auto act = [&](vehicle_part& pt) {
        ...
        if (pt.crew()) { menu.addentry(0, true, 'c', _("Clear assignment")); }
...
void veh_interact::do_rename() {
    std::string name =
        string_input_popup().title(_("Enter new vehicle name:")).width(20).query_string();
    if (!name.empty()) {
        veh->name = name;
        if (veh->tracking_on) {
            get_overmapbuffer(veh->get_dimension()).remove_vehicle(veh);
 ...
 */
int veh_interact::part_at(tripoint_bub_ms d) {
    return veh->part_displayed_at(veh->bubble_to_mount(d));
}
 ...
 */
void veh_interact::move_cursor(tripoint_rel_veh d, int dstart_at) {
    const auto& here = get_map();

    vehicle_cursor += d;
    if (d != tripoint_rel_veh::zero()) {
        start_limit = 0;
        parts_list_offset = 0;
    } else {
        start_at += dstart_at;
    ...
    if (ovp && &ovp->vehicle() != veh) { obstruct = true; }
    // Set obstruction if object is mountable to stop vehicles being built in windows, on tables
    // etc.
    if (here.has_flag(flag_MOUNTABLE, vehp)) {
        obstruct = true;
        // Sanity check that vehicle part is not over a passable but mountable piece of terrain
        if (ovp && &ovp->vehicle() == veh) { obstruct = false; }
    }
    ...
    if (cpart >= 0) {
        ...
        for (size_t i = 0; i < parts_here.size(); i++) {
            ...
            if (pt.base->damage() > 0 && pt.info().is_repairable()) { need_repair.push_back(i); }
            if (pt.info().has_flag("WHEEL")) { wheel = &pt; }
        }
    }
...
void veh_interact::display_grid() {
    ...
    // _|_
    mvwputch(w_border,
             point(getmaxx(w_disp) + 2 + getmaxx(w_list), getmaxy(w_mode) + 2 + page_size),
...
void veh_interact::display_veh() {
    ...
    if (debug_mode) {
        ...
        auto pivot = veh->pivot_point();
        auto com = veh->local_center_of_mass();
        ...
        for (int x = 0; x < getmaxx(w_disp); ++x) {
            if (x <= com_s.x()) { mvwputch(w_disp, point(x, com_s.y()), c_green, LINE_OXOX); }

            if (x >= pivot_s.x()) { mvwputch(w_disp, point(x, pivot_s.y()), c_red, LINE_OXOX); }
        }

        for (int y = 0; y < getmaxy(w_disp); ++y) {
            if (y <= com_s.y()) { mvwputch(w_disp, point(com_s.x(), y), c_green, LINE_XOXO); }

            if (y >= pivot_s.y()) { mvwputch(w_disp, point(pivot_s.x(), y), c_red, LINE_XOXO); }
        }
    }
    ...
    for (auto& structural_part : structural_parts) {
        ...
        const auto part_direction = normalize(
            270_degrees + veh->part_display_direction(p) - veh->face.dir());
        ...
        mvwputch(w_disp, h_size + q.raw(), col, special_symbol(sym));
    }
    ...
    if (cpart >= 0) {
        const auto part_direction = normalize(
            270_degrees + veh->part_display_direction(cpart) - veh->face.dir());
        const auto part_face = tileray(part_direction);
...
void veh_interact::display_veh_tiles() {
    ...
    if (!tile_preview) { tile_preview = std::make_unique<vehicle_preview_window>(); }
...
static std::string wheel_state_description(const vehicle& veh) {
    ...
    if (is_boat) { return string_format(_("Boat: %s"), boat_status); }
...
void veh_interact::display_stats() const {
    ...
    const auto print_stat = [&](auto... args) { print_stat_color(c_light_gray, args...); };
    ...
    if (is_aircraft) {
        print_stat(
            ...
            vel_to_int(veh->max_air_velocity(false, !veh->engine_on)), velocity_units(VU_VEHICLE));
        print_stat(_("Air Acceleration: <color_light_blue>%3d</color> %s/s"),
    ...
    print_stat(_("Mass: <color_light_blue>%5.0f</color> %s"), convert_weight(veh->total_mass()),
               weight_units());
    if (veh->has_lift()) {
        // convert newton to kg.
        units::mass lift_as_mass = units::from_newton(
            veh->total_lift(true, false, !veh->engine_on));
        print_stat(_("Maximum Lift: <color_light_blue>%5.0f</color> %s"),
                   convert_weight(lift_as_mass), weight_units());
        if (veh->has_part("WING")) {
            print_stat(_("Liftoff Speed: <color_light_blue>%3d</color> %s"),
    ...
    if (is_boat) {
        ...
        print_stat(_("Maximum Buoyancy: <color_light_blue>%5.0f</color> %s"),
                   convert_weight(buoyancy_as_mass), weight_units());
    }
    print_stat(
        _("Cargo Volume: <color_light_blue>%s</color> / <color_light_blue>%s</color> %s"),
        format_volume(total_cargo - free_cargo), format_volume(total_cargo), volume_units_abbr());
    ...
    const auto print_part = [&](const std::string& str, int slot, vehicle_part* pt) {
        ...
        return fold_and_print(
            w_stats, point(x[slot] + iw, y[slot]), w[slot], c_light_gray, pt->name());
    };
    ...
    if (mostDamagedPart && mostDamagedPart->damage_percent()) {
        const std::string damaged_header =
            mostDamagedPart == most_repairable
                ? _("Most damaged:")
    ...
    print_stat(_("Air drag:       <color_light_blue>%5.2f</color>"), veh->coeff_air_drag());

    print_stat_if(
        is_boat, _("Water drag:     <color_light_blue>%5.2f</color>"), veh->coeff_water_drag());

    print_stat_if(
        is_ground, _("Rolling drag:   <color_light_blue>%5.2f</color>"), veh->coeff_rolling_drag());

    print_stat(_("Static drag:    <color_light_blue>%5d</color>"), veh->static_drag(false));

    print_stat(
        _("Offroad:        <color_light_blue>%4d</color>%%"),
        static_cast<int>(veh->k_traction(veh->wheel_area() * veh->average_or_rating()) * 100));
    ...
    if (is_boat) {
        ...
        print_stat(draft_string.c_str(), veh->water_draft(), water_clearance);
    }
    ...
    veh->print_fuel_indicators(
        w_stats, point(x[i], fuel_gauge_start_y), fuel_index, true, (x[i] + 10 < getmaxx(w_stats)),
...
static std::string veh_act_desc(
    const input_context& ctxt, const std::string& id, const std::string& desc,
    const task_reason reason) {
    ...
    static const translation inline_fmt_disabled =
        to_translation("keybinding", "<color_dark_gray>%1$s<color_green>%2$s</color>%3$s</color>");
    ...
    static const translation separate_fmt_disabled =
        to_translation("keybinding", "<color_dark_gray><color_green>%1$s</color>-%2$s</color>");
    if (reason == task_reason::CAN_DO) {
        return ctxt.get_desc(
            id, desc, input_context::allow_all_keys, inline_fmt_enabled, separate_fmt_enabled);
    } else {
        return ctxt.get_desc(
            id, desc, input_context::allow_all_keys, inline_fmt_disabled, separate_fmt_disabled);
    }
}
...
void veh_interact::display_mode() {
    ...
    if (title.has_value()) {
        ...
        const std::array<std::string, action_cnt> actions = {{
            ...
            veh_act_desc(main_context, "ASSIGN_CREW", pgettext("veh_interact", "crew"),
                         cant_do('w')),
            veh_act_desc(main_context, "RENAME", pgettext("veh_interact", "rename"),
                         task_reason::CAN_DO),
            veh_act_desc(main_context, "RELABEL", pgettext("veh_interact", "label"), cant_do('a')),
            veh_act_desc(main_context, "CHANGE_SHAPE", pgettext("veh_interact", "shape"),
                         cant_do('p')),
        ...
        for (size_t i = 0; i < action_cnt; i++) {
            ...
            print_colored_text(
                w_mode, point(pos[i] + space * (i + 1) / (action_cnt + 1), 0), dummy, c_white,
                actions[i]);
        }
    }
    wnoutrefresh(w_mode);
 ...
 */
void veh_interact::display_list(
    size_t pos, const std::vector<const vpart_info*>& list, const int header) {
    werase(w_list);
    int lines_per_page = page_size - header;
    size_t page = pos / lines_per_page;
    for (size_t i = page * lines_per_page; i < (page + 1) * lines_per_page && i < list.size();
         i++) {
        ...
        trim_and_print(
            w_list, point(3, y), getmaxx(w_list) - 3, pos == i ? hilite(col) : col, info.name());
    }
    ...
    if (install_info) {
        ...
        for (size_t i = 0; i < tab_list.size(); i++) {
            std::string tab_name = (tab == i) ? tab_list[i] : tab_list_short[i]; // full name for
                                                                                 // selected tab
            tab_x += (tab == i); // add a space before selected tab
            draw_subtab(w_list, tab_x, tab_name, tab == i, false);
            tab_x += (1 + utf8_width(tab_name) + (tab == i)); // one space padding and add a space
                                                              // after selected tab
        }
    }
    wnoutrefresh(w_list);
...
void veh_interact::display_details(const vpart_info* part) {
    ...
    fold_and_print(
        w_details, point(col_1, line + 1), column_width, c_white,
        "%s: <color_light_gray>%d</color>", small_mode ? _("Dur") : _("Durability"),
        part->durability);
    fold_and_print(
        w_details, point(col_2, line + 1), column_width, c_white,
        "%s: <color_light_gray>%d%%</color>", small_mode ? _("Dmg") : _("Damage"), part->dmg_mod);

    // line 2: (column 1) weight   (column 2) folded volume (if applicable)
    fold_and_print(
        w_details, point(col_1, line + 2), column_width, c_white,
        "%s: <color_light_gray>%.1f%s</color>", small_mode ? _("Wgt") : _("Weight"),
        convert_weight(part->item->weight), weight_units());
    if (part->folded_volume != 0_ml) {
        fold_and_print(
            w_details, point(col_2, line + 2), column_width, c_white,
            "%s: <color_light_gray>%s %s</color>", small_mode ? _("FoldVol") : _("Folded Volume"),
            format_volume(part->folded_volume), volume_units_abbr());
    }

    // line 3: (column 1) size, bonus, wheel diameter (if applicable)    (column 2) epower, wheel
    // width (if applicable)
    if (part->size > 0_ml && part->has_flag(VPFLAG_CARGO)) {
        fold_and_print(
            w_details, point(col_1, line + 3), column_width, c_white,
    ...
    if (part->bonus > 0) {
        ...
        if (part->has_flag(VPFLAG_SEATBELT)) {
            ...
            part->has_flag(VPFLAG_LIGHT) || part->has_flag(VPFLAG_CONE_LIGHT)
        ...
        if (!label.empty()) {
            fold_and_print(w_details, point(col_1, line + 3), column_width, c_white,
                           "%s: <color_light_gray>%d</color>", label, part->bonus);
        }
    }
    ...
    if (part->has_flag(VPFLAG_WHEEL)) {
        ...
        fold_and_print(
            w_details, point(col_1, line + 3), column_width, c_white,
            "%s: <color_light_gray>%s</color>", small_mode ? _("Dia") : _("Wheel Diameter"),
            wheel_dimensions::format_for_display(whl ? whl->diameter : 0).c_str());
        fold_and_print(
            w_details, point(col_2, line + 3), column_width, c_white,
            "%s: <color_light_gray>%s</color>", small_mode ? _("Wdt") : _("Wheel Width"),
    ...
    if (part->epower != 0) {
        fold_and_print(
            w_details, point(col_2, line + 3), column_width, c_white,
            "%s: <color_light_gray>%+4d</color>", small_mode ? _("Epwr") : _("Electric Power"),
            part->epower);
    }

    // line 4 [horizontal]: fuel_type (if applicable)
    // line 4 [vertical/hybrid]: (column 1) fuel_type (if applicable)    (column 2) power (if
    // applicable) line 5 [horizontal]: power (if applicable)
    if (!part->fuel_type.is_null()) {
        fold_and_print(w_details, point(col_1, line + 4), column_width, c_white,
                       _("Charge: <color_light_gray>%s</color>"), item::nname(part->fuel_type));
    }
    ...
    for (size_t i = 0; i < flags.size(); i++) {
        if (part->has_flag(flags[i])) { label += (label.empty() ? "" : " ") + flag_labels[i]; }
    }
    ...
    if (part->fuel_type == itype_battery && !part->has_flag(VPFLAG_ENGINE)
        ...
        fold_and_print(
            w_details, point(col_2, line + 5), column_width, c_white,
            "%s: <color_light_gray>%8d</color>", small_mode ? _("BatCap") : _("Battery Capacity"),
            battery->capacity);
    } else {
...
void act_vehicle_unload_fuel(vehicle* veh) {
    ...
    if (fuels.size() > 1) {
        ...
        for (size_t i = 0; i < fuels.size(); i++) {
            ...
            if (fuel == itype_plut_cell && veh->fuel_left(fuel) < PLUTONIUM_CHARGES) { continue; }
            smenu.entries.emplace_back(
                uilist_entry(item::nname(fuel)).with_retval(static_cast<int>(i)));
        }
        smenu.query();
    ...
    switch (static_cast<char>(who.activity->index)) {
        case 'i': {
            ...
            if (vpinfo.has_flag(VPFLAG_CONE_LIGHT) || vpinfo.has_flag(VPFLAG_WIDE_CONE_LIGHT)
                ...
                do {
                    popup(
                        _("Press space, choose a facing direction for the new %s and confirm with enter."),
                        vpinfo.name());

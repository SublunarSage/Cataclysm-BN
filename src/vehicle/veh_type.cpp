...
auto vpart_rotating_light::beam_count() const -> int { return std::max(beams, 1); }

auto vpart_rotating_light::beam_spacing() const -> units::angle {
    return units::from_degrees(360.0 / static_cast<double>(beam_count()));
}

auto vpart_rotating_light::direction_at(
    const units::angle base_direction, const time_point turn) const -> units::angle {
    ...
    const auto state_count =
        step_magnitude > 0 ? static_cast<int>(360 / std::gcd(360LL, rotated_beam_step)) : 1;
...
static auto load_rotating_light(
    const JsonObject& jo, std::optional<vpart_rotating_light>& rotating_light) -> void {
    if (!jo.has_member("rotating_light")) { return; }
    ...
    if (jrot.has_int("period")) {
        ...
        data.period =
            read_from_json_string<time_duration>(*jrot.get_raw("period"), time_duration::units);
    }

    if (data.period <= 0_turns) { jrot.throw_error("period must be positive", "period"); }
...
static const std::vector<std::pair<std::string, int>> off_road_terrain_mod = {
    {{"FLAT", 3}, {"ROAD", 1}}};
static const std::vector<std::pair<std::string, int>> treads_terrain_mod = {{{"FLAT", 3}}};
static const std::vector<std::pair<std::string, int>> rail_terrain_mod = {{{"RAIL", 8}}};

static void parse_vp_reqs(
    const JsonObject& obj, const std::string& id, const std::string& key,
    std::vector<std::pair<requirement_id, int>>& reqs, std::map<skill_id, int>& skills,
    int& moves) {

    if (!obj.has_object(key)) { return; }
    JsonObject src = obj.get_object(key);

    auto sk = src.get_array("skills");
    if (!sk.empty()) { skills.clear(); }
    ...
    if (src.has_int("time")) {
        ...
        moves = to_moves<int>(
            read_from_json_string<time_duration>(*src.get_raw("time"), time_duration::units));
    }
...
void vpart_info::load_engine(
    ...
    if (!excludes.empty()) {
        ...
        for (const std::string line : excludes) { e_info.exclusions.push_back(line); }
    }
    auto fuel_opts = jo.get_array("fuel_options");
    if (!fuel_opts.empty()) {
        e_info.fuel_opts.clear();
        for (const std::string line : fuel_opts) { e_info.fuel_opts.emplace_back(line); }
...
void vpart_info::load_propeller(std::optional<vpslot_propeller>& proptr, const JsonObject& jo) {
    ...
    if (proptr) { propeller_info = *proptr; }
...
void vpart_info::load_crafter(std::optional<vpslot_crafter>& craftptr, const JsonObject& jo) {
    ...
    if (craftptr) { craft_info = *craftptr; }

    JsonArray tools = jo.get_array("integrated_tools");

    for (std::string cur : tools) { craft_info.fake_parts.emplace_back(itype_id(cur)); }
...
void vpart_info::load_converter(std::optional<vpslot_converter>& convertptr, const JsonObject& jo) {
    ...
    if (convertptr) { convert_info = *convertptr; }
...
void vpart_info::load(const JsonObject& jo, const std::string& src) {
    ...
    if (jo.has_member("transform_terrain")) {
        ...
        if (jttd.has_int("post_field_age")) {
            ...
            transform_terrain.post_field_age = read_from_json_string<
                time_duration>(*jttd.get_raw("post_field_age"), time_duration::units);
        } else {
    ...
    if (jo.has_member("symbol")) { sym = jo.get_string("symbol")[0]; }
    if (jo.has_member("broken_symbol")) { sym_broken = jo.get_string("broken_symbol")[0]; }
    ...
    if (jo.has_member("color")) { color = color_from_string(jo.get_string("color")); }
    ...
    if (!qual.empty()) {
        qualities.clear();
        for (JsonArray pair : qual) { qualities[quality_id(pair.get_string(0))] = pair.get_int(1); }
    }
    ...
    if (has_flag("ENGINE")) { load_engine(engine_info, jo, fuel_type); }

    if (has_flag("WHEEL")) { load_wheel(wheel_info, jo); }

    if (has_flag("ROTOR")) { load_rotor(rotor_info, jo); }

    if (has_flag("PROPELLER")) { load_propeller(propeller_info, jo); }

    if (has_flag("BALLOON")) { load_balloon(balloon_info, jo); }

    if (has_flag("LADDER")) { load_ladder(ladder_info, jo); }

    if (has_flag("WING")) { load_wing(wing_info, jo); }

    if (has_flag("CONVERTER")) { load_converter(converter_info, jo); }

    if (has_flag("WORKBENCH")) { load_workbench(workbench_info, jo); }

    if (has_flag("CRAFTER")) { load_crafter(crafter_info, jo); }
    ...
    if (jo.has_array("shapes")) {
        for (JsonObject obj : jo.get_array("shapes")) {
            ...
            if (obj.has_string("symbol")) { shape.sym = obj.get_string("symbol")[0]; }
            if (obj.has_string("symbol_broken")) { shape.sym = obj.get_string("symbol_broken")[0]; }
            if (obj.has_string("looks_like")) { shape.looks_like = obj.get_string("looks_like"); }
...
void vpart_info::set_flag(const std::string& flag) {
    ...
    if (iter != vpart_bitflag_map.end()) { bitflags.set(iter->second); }
}
...
void vpart_info::finalize() {
    if (folded_volume > 0_ml) { set_flag("FOLDABLE"); }

    for (const auto& f : flags) {
        auto b = vpart_bitflag_map.find(f);
        if (b != vpart_bitflag_map.end()) { bitflags.set(b->second); }
    }
    ...
    if (removal_moves < 0) { removal_moves = install_moves / 2; }
}

void vpart_info::check_consistency() { all_vparts.check(); }
...
void vpart_info::check() const {
    ...
    if (removal_moves < 0) { debugmsg("vehicle part %s has negative removal time", id.c_str()); }

    if (!item_group::group_is_defined(breaks_into_group)) {
        debugmsg("Vehicle part %s breaks into non-existent item group %s.", id.c_str(),
                 breaks_into_group.c_str());
    }
    if (sym == 0) { debugmsg("vehicle part %s does not define a symbol", id.c_str()); }
    if (sym_broken == 0) {
    ...
    if (dmg_mod < 0) { debugmsg("vehicle part %s has negative damage modifier", id.c_str()); }
    ...
    if (size < 0_ml) { debugmsg("vehicle part %s has negative size", id.c_str()); }
    if (!item.is_valid()) {
    ...
    if (!fuel_type.is_valid()) {
        throw JsonError(
            string_format("vehicle part %s uses undefined fuel %s", id.c_str(), fuel_type.c_str()));
        ...
        throw JsonError(string_format(
            "non-tank vehicle part %s uses non-fuel item %s as fuel, setting to null", id.c_str(),
            fuel_type.c_str()));
    }
    if (has_flag("TURRET") && !base_item_type.gun) {
        debugmsg("vehicle part %s has the TURRET flag, but is not made from a gun item",
                 id.c_str());
    }
    ...
    if (has_flag("EMITTER")) {
        if (emissions.empty()) {
            ...
            for (const emit_id& e : emissions) {
                if (!e.is_valid()) {
                    debugmsg(
                        "vehicle part %s has the EMITTER flag, but invalid emission %s was set",
    ...
    static const std::vector<std::string> handled = {
        {"ENABLED_DRAINS_EPOWER", "SECURITY", "ENGINE", "ALTERNATOR", "SOLAR_PANEL",
         "POWER_TRANSFER", "PERPETUAL", "REACTOR", "WIND_TURBINE", "WATER_WHEEL"}};
    if (epower != 0 && std::ranges::none_of(handled, [*this](const std::string& flag) {
            return has_flag(flag);
        })) {
        std::string warnings_are_good_docs = enumerate_as_string(handled);
        debugmsg(
            "%s has non-zero epower, but lacks a flag that would make it affect epower (one of %s)",
...
int vpart_info::format_description(
    ...
    if (!description.empty()) { long_descrip += description.translated(); }
    for (const auto& flagid : flags) {
        if (flagid == "ALARMCLOCK" || flagid == "WATCH") { continue; }
        json_flag flag = json_flag::get(flagid);
        if (!flag.info().empty()) {
            if (!long_descrip.empty()) { long_descrip += "  "; }
    ...
    if (has_flag("TURRET")) {
        ...
        if (base.ammo_required() && !base.ammo_remaining()) {
            itype_id default_ammo =
                base.magazine_current() ? base.common_ammo_default() : base.ammo_default();
            base.ammo_set(default_ammo);
        }
        long_descrip += string_format(
    ...
    if (!long_descrip.empty()) {
        ...
        msg += wrap_descrip[0];
        for (size_t i = 1; i < wrap_descrip.size(); i++) { msg += "\n  " + wrap_descrip[i]; }
        msg += "</color>\n";
    ...
    for (const auto& qual : qualities) {
        msg += string_format(
            _("Has level <color_cyan>%1$d %2$s</color> quality"), qual.second,
            qual.first.obj().name);
        if (qual.first == quality_jack || qual.first == quality_lift) {
            msg += string_format(
                _(" and is rated at <color_cyan>%1$d %2$s</color>"),
                static_cast<int>(convert_weight(qual.second * TOOL_LIFT_FACTOR)), weight_units());
        }
        msg += ".\n";
        lines += 1;
    }
...
static int scale_time(const std::map<skill_id, int>& sk, int mv, const Character& who) {
    ...
    const int lvl = std::
        accumulate(sk.begin(), sk.end(), 0, [&who](int lhs, const std::pair<skill_id, int>& rhs) {
            return lhs
                 + std::max(std::min(who.get_skill_level(rhs.first), MAX_SKILL) - rhs.second, 0);
        });
    // 10% per excess level (reduced proportionally if >1 skill required) with max 50% reduction
    // 10% reduction per assisting NPC
    int time_norm =
        mv * (1.0 - std::min(static_cast<double>(lvl) / sk.size() / 10.0, 0.5))
        * (10 - character_funcs::get_crafting_helpers(who, 3).size()) / 10;
    time_norm +=
        who.bonus_from_enchantments(time_norm, enchantment_value_id("CONSTRUCTION_SPEED_VEH"));
    return time_norm;
}
...
template <> const vehicle_prototype& string_id<vehicle_prototype>::obj() const {
    ...
    if (iter == vtypes.end()) {
        ...
        static const vehicle_prototype dummy =
            {"", std::vector<vehicle_prototype::part_def>{}, std::vector<vehicle_item_spawn>{},
             nullptr};
        return dummy;
    }
    return iter->second;
...
void vehicle_prototype::load(const JsonObject& jo) {
    ...
    if (!vproto.parts.empty() && jo.has_string("name")) { vproto = vehicle_prototype(); }
    if (vproto.parts.empty()) { vproto.name = jo.get_string("name"); }
    ...
    if (jo.has_member("flags")) { vproto.flags = jo.get_tags<flag_id>("flags"); }
    ...
    for (JsonObject part : jo.get_array("parts")) {
        auto pos = tripoint_mnt_veh(
            part.get_int("x"), part.get_int("y"), part.has_int("z") ? part.get_int("z") : 0);
    ...
    for (JsonObject spawn_info : jo.get_array("items")) {
        ...
        if (next_spawn.chance <= 0 || next_spawn.chance > 100) {
            if (do_z) {
                debugmsg("Invalid spawn chance in %s (%d, %d, %d): %d%%", vproto.name,
                         next_spawn.pos.x(), next_spawn.pos.y(), next_spawn.pos.z(),
                         next_spawn.chance);
            } else {
                debugmsg("Invalid spawn chance in %s (%d, %d): %d%%", vproto.name,
                         next_spawn.pos.x(), next_spawn.pos.y(), next_spawn.chance);
            }
        }

        // constrain both with_magazine and with_ammo to [0-100]
        next_spawn.with_magazine =
            std::max(std::min(spawn_info.get_int("magazine", next_spawn.with_magazine), 100), 0);
        next_spawn.with_ammo =
            std::max(std::min(spawn_info.get_int("ammo", next_spawn.with_ammo), 100), 0);
...
void vehicle_prototype::reset() { vtypes.clear(); }
    ...
    for (auto& vp : vtypes) {
        ...
        for (auto& pt : proto.parts) {
            if (proto.color_palette.is_valid() && !proto.color_match.contains(pt.part.str())) {
                ...
                if (index != -1) { proto.color_match[pt.part.str()] = index; }
            }
            ...
            if (blueprint.install_part(pt.pos, pt.part, true) < 0) {
                debugmsg("init_vehicles: '%s' part '%s'(%d) can't be installed to %d,%d",
                         blueprint.name, pt.part.c_str(), blueprint.part_count(), pt.pos.x(),
                         pt.pos.y());
            }
            ...
            if (!base->gun) {
                ...
                if (!pt.ammo_types.empty()) {
                    debugmsg("init_vehicles: non-turret %s with ammo_types in %s", pt.part.c_str(),
                             id.c_str());
                }
                if (pt.ammo_qty.first > 0 || pt.ammo_qty.second > 0) {
                    debugmsg("init_vehicles: non-turret %s with ammo_qty in %s", pt.part.c_str(),
                             id.c_str());
                }
                ...
                if (pt.ammo_types.empty()) {
                    if (!base->gun->ammo.empty()) {
                        pt.ammo_types.insert(
                            ammotype(*base->gun->ammo.begin())->default_ammotype());
                    }
                }
            }
            ...
            if (base->container || base->magazine) {
                if (!pt.fuel.is_valid()) {
                    debugmsg("init_vehicles: tank %s specified invalid fuel in %s", pt.part.c_str(),
                             id.c_str());
                }
            } else {
                if (!pt.fuel.is_null()) {
            ...
            if (pt.part.obj().has_flag("CARGO")) { cargo_spots.insert(pt.pos); }
        }

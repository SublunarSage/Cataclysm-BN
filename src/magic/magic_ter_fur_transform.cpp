...
template <class T>
static void load_transform_results(
    const JsonObject& jsi, const std::string& json_key, weighted_int_list<T>& list) {
...
template <class T> void ter_furn_data<T>::load(const JsonObject& jo) {
...
void ter_furn_transform::load(const JsonObject& jo, const std::string&) {
    ...
    if (jo.has_member("terrain")) {
        for (JsonObject ter_obj : jo.get_array("terrain")) {
            ...
            if (ter_obj.get_bool("diggable", false)) { diggable_ter_transform = cur_results; }
        }
    }
...
void ter_furn_transform::add_all_messages(
...
void ter_furn_transform::add_all_messages(
    ...
    if (!add_message(ter_transform, ter_at_loc->id, critter, location)) {
        for (const std::pair<const std::string, ter_furn_data<ter_str_id>>& data :
             ter_flag_transform) {
    ...
    if (!add_message(furn_transform, furn_at_loc->id, critter, location)) {
        for (const std::pair<const std::string, ter_furn_data<furn_str_id>>& data :
             furn_flag_transform) {
    ...
    if (!furn_potential) {
        for (const std::pair<const std::string, ter_furn_data<furn_str_id>>& flag_result :
            ...
            if (furn_at_loc->has_flag(flag_result.first)) {
                ...
                if (furn_potential) { break; }
            }
        }
    }

    if (ter_potential) { m.ter_set(location, *ter_potential); }
    if (furn_potential) { m.furn_set(location, *furn_potential); }
}

auto ter_furn_transform::transform(mapgen_constructor& m, const point_omt_ms& location) const
    -> void {
    const ter_id ter_at_loc = m.ter(location);
    std::optional<ter_str_id> ter_potential = next_ter(ter_at_loc->id);
    const furn_id furn_at_loc = m.furn(location);
